"""D8: the large band, where new BKS is still possible.

D7 established that on the small band the pooled mechanism converges exactly onto
the 2013 BKS and cannot pass it -- PR17B and PR21B land bit-exactly on it under
640,000 total iterations, which suggests those BKS are optimal.  The large band
(840-960 clients) is different: we sit 1.4-2.3% above BKS there, so those
instances are demonstrably not solved to optimality and room exists.

The large band has never been given long runs.  Everything measured on it so far
used 6,200 iterations, against 16,000-64,000 on the small band.  This applies the
same monotone-safe pooling mechanism at 20,000 iterations x 10 seeds.

Budget note: PR16A runs about 12.6 iterations per second, so 20,000 iterations is
roughly 26 minutes per run; 40 runs over 6 workers is roughly 3 hours.  That is
the reason for 20,000 rather than D7's 64,000 -- at 64,000 this band would need
about 56 hours.

Original D7 docstring follows.
---
D7: the untested cell -- long runs, many seeds, pooled.

P4 already sprinted for new BKS with roughly 5x the budget but only two seeds,
and got zero.  D6 pooled ten seeds but each run was short (16,000 iterations),
and reached BKS exactly on PR17B without passing it.  Long x many x pooled is
the combination nobody has run.

This is more compute on a mechanism already shown to be monotone-safe -- the
incumbent's routes are always in the pool, so the partition can never return
something worse than best-of-k.  It is not a new mechanism and is not claimed
as one.

Original D6 docstring follows.
---
...
SCALE = 1000.0
ITERS = 20000
K_NOIMPROVE = 5000  # same ratio as D6 (16000/4000)
SP_TIME = 300.0  # larger instances, more columns
EPS = 1.0e-6
...
def main() -> int:
    ...
    with (OUT / "d8_raw_runs.csv").open("w", newline="", encoding="utf-8") as h:
    ...
    summary = {
        "schema": "resetp.public-algo-diag.d8.v1",
    ...
    (OUT / "d8_summary.json").write_text(
