#!/usr/bin/env python3
"""Fresh-case gate for MV-HGS-SP route-pool recombination."""
...
PREREG = PACKAGE / "h5_fresh_route_pool_preregistration.json"
OUT = PACKAGE / "h5_fresh_route_pool_gate"
...
def main() -> int:
    ...
    passed = losses == 0 and wins >= 2 and all_feasible and tests_passed
    decision = {
        "schema_version": (
            "resetp.china81-h5-fresh-route-pool-decision.v1"
        ),
        "decision": (
            "PASS_FRESH_MV_HGS_SP_ARCHITECTURE"
            if passed
            else "STOP_FRESH_MV_HGS_SP_ARCHITECTURE"
        ),
        "passed": passed,
        "wins": wins,
        "ties": ties,
        ...
        "next_gate": (
            "freeze_architecture_and_multiseed_confirmation"
            if passed
            else "stop_current_route_pool_architecture"
        ),
    }
    metadata = {
        "schema_version": (
            "resetp.china81-h5-fresh-route-pool-metadata.v1"
        ),
