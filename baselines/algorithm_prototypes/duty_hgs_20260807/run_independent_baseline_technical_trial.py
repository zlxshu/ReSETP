...
def _run_depot(bundle, full_individual, depot_id: str, iterations: int):
    ...
    candidates, initial_evaluation, _reverse, attempts, _selected = (
        _prepare_population(
            initial,
            evaluator,
            policy,
...
def _report(rows: list[dict[str, Any]], instance_id: str) -> str:
    passed = [
...
`{instance_id}` 的 {len(rows)} 个车场已串行完成各一个搜索循环，{len(passed)}/{len(rows)} 个通过单车场求解、利润计算、完整检查和逐候选真值哨兵。{details}
...
def main() -> int:
    ...
    _json(
        ...
        {
            "schema": "resetp.duty-hgs-independent-baseline-trial.v1",
