...
`cn-prd-150c-01-V2-LOCATIONS` 共有 3 名客户在已保存的零搜索机会表中由其他车场直接服务更近。现有动作生成器围绕这些客户生成 782 个跨车场候选，其中 10 个进入完整评价，0 个通过完整模型，0 个相对登记起点的罚后目标更优。完整真值复核执行 10 次，耗时 1.590 秒，进程峰值内存 62.438 MB。
