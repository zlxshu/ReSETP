...
`cn-prd-150c-03-V2-LOCATIONS` 共有 6 名客户在已保存的零搜索机会表中由其他车场直接服务更近。现有动作生成器围绕这些客户生成 1367 个跨车场候选，其中 20 个进入完整评价，0 个通过完整模型，0 个相对登记起点的罚后目标更优。完整真值复核执行 20 次，耗时 3.104 秒，进程峰值内存 106.344 MB。
