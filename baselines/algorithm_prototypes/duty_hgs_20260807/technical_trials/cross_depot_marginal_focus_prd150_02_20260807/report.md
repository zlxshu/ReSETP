...
`cn-prd-150c-02-V2-LOCATIONS` 共有 2 名客户在已保存的“capacity-feasible route-marginal relocation”筛查中距离变化为负。现有动作生成器围绕这些客户生成 436 个跨车场候选，其中 8 个进入完整评价，0 个通过完整模型，0 个相对登记起点的罚后目标更优。完整真值复核执行 8 次，耗时 2.155 秒，进程峰值内存 62.406 MB。
