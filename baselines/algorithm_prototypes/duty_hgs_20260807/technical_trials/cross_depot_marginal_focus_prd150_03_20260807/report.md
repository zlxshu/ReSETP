...
`cn-prd-150c-03-V2-LOCATIONS` 共有 4 名客户在已保存的“capacity-feasible route-marginal relocation”筛查中距离变化为负。现有动作生成器围绕这些客户生成 872 个跨车场候选，其中 19 个进入完整评价，0 个通过完整模型，0 个相对登记起点的罚后目标更优。完整真值复核执行 19 次，耗时 4.090 秒，进程峰值内存 81.891 MB。
