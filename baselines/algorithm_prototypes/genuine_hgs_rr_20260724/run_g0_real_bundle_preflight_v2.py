#!/usr/bin/env python3
"""Run the versioned G0 real-bundle gate after the registered decoder fix."""
...
AMENDMENT = PACKAGE / "g0_real_bundle_preregistration_v2.json"
...
def load_preregistration_v2() -> dict[str, Any]:
    ...
    if (
        ...
        != "resetp.coop-hgs-rr-g0-real-bundle-amendment.v2"
    ):
        raise RuntimeError("unexpected G0 v2 amendment schema")
    if amendment.get("status") != "FROZEN_BEFORE_G0_V2_EXECUTION":
        raise RuntimeError("G0 v2 amendment status drift")
    ...
    for relative, expected in amendment["source_hashes"].items():
        ...
        if not path.is_file():
            raise RuntimeError(f"G0 v2 source missing: {relative}")
        actual = frozen.sha256(path)
        if actual != str(expected):
            raise RuntimeError(
                f"G0 v2 source drift: {relative}:{expected}:{actual}"
            )
...
frozen.OUT = PACKAGE / "g0_real_bundle_gate_v2"
frozen.load_preregistration = load_preregistration_v2
