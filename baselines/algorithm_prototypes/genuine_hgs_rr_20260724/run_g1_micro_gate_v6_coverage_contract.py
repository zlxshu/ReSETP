#!/usr/bin/env python3
"""Run fresh G1 after the narrowed route-local coverage repair."""
...
AMENDMENT = PACKAGE / "g1_micro_preregistration_v6_coverage_contract.json"
BASE_V3 = PACKAGE / "g1_micro_preregistration_v3.json"
BASE_V4 = PACKAGE / "g1_micro_preregistration_v4_mechanical.json"
G0_V3_REGISTRATION = PACKAGE / "g0_real_bundle_preregistration_v3.json"
G0_V3_GATE = PACKAGE / "g0_real_bundle_gate_v3"
WORKER_PROBE_V3 = PACKAGE / "g1_six_worker_resource_probe_v3"
MECHANICAL_GATE = (
    REPO
...
def load_preregistration_v6() -> dict[str, Any]:
    amendment = read_json(AMENDMENT)
    if (
        amendment.get("schema")
        != "resetp.coop-hgs-rr-g1-preregistration-amendment.v6"
    ):
        raise RuntimeError("unexpected G1 v6 amendment schema")
    if amendment.get("status") != "FROZEN_BEFORE_G1_V6_RESULTS":
        raise RuntimeError("G1 v6 amendment status drift")
    for path, field in (
        (BASE_V3, "base_v3_sha256"),
        (BASE_V4, "base_v4_sha256"),
        (G0_V3_REGISTRATION, "g0_v3_registration_sha256"),
    ):
        if frozen.sha256(path) != amendment[field]:
            raise RuntimeError(f"G1 v6 registered evidence drift: {path.name}")
    for relative, expected in amendment["execution_source_hashes"].items():
        path = REPO / relative
        if not path.is_file():
            raise RuntimeError(f"G1 v6 source missing: {relative}")
        actual = frozen.sha256(path)
        if actual != expected:
            raise RuntimeError(
                f"G1 v6 source drift: {relative}:{expected}:{actual}"
            )
...
frozen.G0_PREREGISTRATION = G0_V3_REGISTRATION
frozen.G0_GATE = G0_V3_GATE
frozen.WORKER_PROBE = WORKER_PROBE_V3
frozen.WORK = PACKAGE / "g1_micro_work_v6_coverage_contract"
frozen.OUT = PACKAGE / "g1_micro_gate_v6_coverage_contract"
frozen.RELEASE_DECISION = MECHANICAL_GATE / "decision.json"
frozen.RELEASE_VERDICT = "PASS_E2_V7_MECHANICAL_INTEGRITY_RELEASE"
frozen.real_bundle_gate = g0_v3.frozen
frozen.load_preregistration = load_preregistration_v6
...
def write_done() -> None:
    ...
    frozen.write_json(
        ...
        {
            "schema": "resetp.coop-hgs-rr-g1-done.v6",
