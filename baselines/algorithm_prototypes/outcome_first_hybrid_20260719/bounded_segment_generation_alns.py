"""One continuous project ALNS with optional bounded SEG-GEN-02 moves."""
...
from bounded_segment_generator import (  # noqa: E402
    BoundedSegmentGenerationController,
...
class BoundedSegmentAlnsConfig:
...
def run_bounded_segment_generation_alns(
    bundle_dir: str | Path,
    *,
    seed: int,
    config: BoundedSegmentAlnsConfig | None = None,
    prices: Any = DEFAULT_PRICES,
    initial_solution: Solution | None = None,
) -> ArmResult:
    """Run one same-loop control, SEG-GEN-02, or distance-ablation arm."""

    cfg = config or BoundedSegmentAlnsConfig()
    ...
    if cfg.mode != "disabled":
        controller = BoundedSegmentGenerationController(
            BoundedSegmentGeneratorConfig(
                mode=cfg.mode,
                ...
                max_positions_per_source=int(
                    cfg.max_positions_per_source
                ),
                max_local_exact_candidates=int(
                    cfg.max_local_exact_candidates
                ),
                max_completed_candidates=int(
                    cfg.max_completed_candidates
                ),
    ...
    if not (
        ...
        raise RuntimeError(
            "SEG-GEN-02 complete-candidate ledger did not close"
        )
    if not bool(selector.get("selection_count_closed", False)):
        raise RuntimeError("SEG-GEN-02 selector count did not close")
    ...
    if mechanism_evaluations != mechanism_moves:
        raise RuntimeError(
            "SEG-GEN-02 move and evaluation ledgers disagree"
        )
    if (
        ...
        raise RuntimeError(
            "generic plus SEG-GEN-02 moves do not close budget"
        )
    ...
    if cfg.apply_terminal_completion:
        ...
        if not completed.feasible:
            raise RuntimeError(
                "SEG-GEN-02 terminal completion infeasible"
            )
    ...
    replay = independent_cost(bundle_dir, final_solution, prices)
    ...
    if violations:
        raise RuntimeError(
            f"SEG-GEN-02 final infeasible: {violations[:8]}"
        )
    ...
    return ArmResult(
        algorithm={
            ...
            "mechanism": "bounded_segment_generation_mechanism_alns",
            "distance": "bounded_segment_generation_distance_ablation",
        }[cfg.mode],
