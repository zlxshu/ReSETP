#!/usr/bin/env python3
"""Freeze corrected fresh route-pair inputs; preserve invalid v1."""
...
V1 = PACKAGE / "input_selection_v1.json"
OUTPUT = PACKAGE / "input_selection_v2.json"
MANIFEST_ROOT = (
    REPO
...
EXCLUDED = {
    ...
    # Used by the sealed genuine-hybrid G1 v6.
...
def main() -> int:
    ...
    payload = {
        "schema": "resetp.unordered-route-pair-input-selection.v2",
        "status": "FROZEN_BEFORE_OBJECTIVE_ACCESS",
        "registered_at_utc": datetime.now(timezone.utc).isoformat(),
        "supersedes": {
            "path": V1.relative_to(REPO).as_posix(),
            "sha256": file_digest(V1),
            "reason": (
                "v1 omitted cn-cy-150c-03-V2-LOCATIONS from the prior-use "
                "exclusion list; the omission was found before objective "
                "access or algorithm execution."
            ),
        },
        "selection_rule": (
