#!/usr/bin/env python3
"""Independent re-reader and complete-model verifier for IND/ZONE/JOINT."""
...
def main() -> int:
    ...
    repo = out.parents[2]
    runner_path = out / "run_e3_mismatch.py"
    runner = load_module("_e3_mismatch_checker_runner", runner_path)
    ...
    expected = (
        len(runner.INSTANCES) * len(runner.ARMS) * len(runner.SEEDS)
    )
    if len(rows) != expected:
        raise RuntimeError(
            f"HALT_CHECKER_DENOMINATOR:{len(rows)}/{expected}"
        )
    keys = {
    ...
    for row in rows:
        ...
        if arm in {"IND", "ZONE"} and (
            audit["cross_site_service_count"] != 0
        ):
    ...
    for instance_id, _role in runner.INSTANCES:
        ...
        if administrative != expected_administrative:
            raise RuntimeError(
                f"HALT_CHECKER_IND_MAPPING:{instance_id}"
            )
        if zone != expected_zone:
            raise RuntimeError(
                f"HALT_CHECKER_ZONE_MAPPING:{instance_id}"
            )
        if administrative == zone:
            raise RuntimeError(
                f"HALT_CHECKER_EXPECTED_NONZERO_MISMATCH:{instance_id}"
            )
    proof = json.loads(
        runner.UPSTREAM_JOINT_PROOF.read_text(encoding="utf-8")
    )
    if proof.get("status") != "PASS_JOINT_SEMANTICS_UNCHANGED":
        raise RuntimeError("HALT_CHECKER_JOINT_PROOF")
    bug_fix = json.loads(
        runner.UPSTREAM_BUG_FIX.read_text(
            encoding="utf-8"
        )
    )
    if (
    ...
    result = {
        "schema":
            "resetp.e3-mismatch-restart.independent-verification.v1",
        ...
        "administrative_zone_nonidentity_rechecked": True,
    ...
    print(
        f"PASS_INDEPENDENT_VERIFICATION formal_units_checked={expected}",
        flush=True,
    )
    return 0
