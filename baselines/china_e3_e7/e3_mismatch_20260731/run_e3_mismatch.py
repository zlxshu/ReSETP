#!/usr/bin/env python3
"""Run the journal-aligned E3 IND/ZONE/JOINT mismatch comparison.
...
CHECKER = HERE / "check_e3_mismatch.py"
PREREGISTRATION = HERE / "pre_registration.json"
INPUT_AUDIT_SUMMARY = HERE / "input_audit/summary.json"
INPUT_AUDIT_CUSTOMERS = HERE / "input_audit/mismatched_customers.csv"
UPSTREAM_SOURCE_LOCK = (
    REPO / "baselines/china_e3_e7/e3_zone_joint_20260731/source_lock.json"
)
UPSTREAM_BUG_FIX = (
    REPO
    / "baselines/china_e3_e7/e3_zone_joint_20260731/"
    "cross_site_bug_fix_evidence.json"
)
UPSTREAM_JOINT_PROOF = (
    REPO
    / "baselines/china_e3_e7/e3_zone_joint_20260731/"
    "joint_semantics_proof.json"
)
TASK_ID = "E3-MISMATCH-RESTART-20260731"
INSTANCES = (
    ("cn-prd-200c-01-V2-LOCATIONS", "MAIN_DISPLAY"),
    ("cn-prd-200c-02-V2-LOCATIONS", "ROBUSTNESS"),
    ("cn-prd-150c-03-V2-LOCATIONS", "ROBUSTNESS"),
    ("cn-prd-150c-01-V2-LOCATIONS", "ROBUSTNESS"),
    ("cn-prd-150c-02-V2-LOCATIONS", "ROBUSTNESS"),
    ("cn-prd-200c-03-V2-LOCATIONS", "ROBUSTNESS"),
)
ARMS = ("IND", "ZONE", "JOINT")
...
LOCKED_SOURCES = (
    ...
    CHECKER,
    HERE / "audit_mismatch.py",
    PREREGISTRATION,
    INPUT_AUDIT_SUMMARY,
    INPUT_AUDIT_CUSTOMERS,
    UPSTREAM_SOURCE_LOCK,
    UPSTREAM_BUG_FIX,
    UPSTREAM_JOINT_PROOF,
    CONTRACT,
...
def build_inputs(workers: int) -> dict[str, Any]:
    ...
    if (
        ...
        or int(
            preregistration.get(
                "search_evaluations_observed_before_registration", -1
            )
        )
        != 0
    ):
        raise RuntimeError("HALT_PREREGISTRATION_NOT_LOCKED")
    ...
    investigation = {
        ...
        "ind_arm_run": True,
        "ind_arm_reason":
            "ALL_SELECTED_INPUTS_HAVE_NONZERO_ADMINISTRATIVE_ROAD_MISMATCH",
        "instances": field_evidence,
        "input_audit_summary": relative(INPUT_AUDIT_SUMMARY),
    }
    atomic_json(HERE / "depot_field_investigation.json", investigation)
    prepared = {
        "schema": "resetp.e3-mismatch-restart.prepare.v1",
        ...
        "instances": [instance_id for instance_id, _ in INSTANCES],
        "formal_units_expected": len(INSTANCES) * len(ARMS) * len(SEEDS),
        "pre_registration_sha256": file_sha256(PREREGISTRATION),
...
def run_probe(cap: int, workers: int) -> dict[str, Any]:
    ...
    if status["termination_reason"] == "CANDIDATES_EXHAUSTED":
        ...
        basis_eval = cap
        selected = cap
    selected = min(cap, max(400, selected))
    atomic_csv(HERE / "probe/convergence_curve.csv", curve)
    summary = {
        "schema": "resetp.e3-mismatch-restart.convergence-probe.v1",
        "status": "PASS",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
        "instance_id": spec["instance_id"],
    ...
    lock = verify_source_lock()
    budget_lock = {
        "schema": "resetp.e3-mismatch-restart.budget-lock.v1",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
        "budget_semantics": "UPPER_CAP_NOT_QUOTA",
        "budget_source":
            "JOINT_LARGEST_HIGHEST_MISMATCH_INSTANCE_SEED1_CONVERGENCE_PROBE",
        "formal_cap_floor_from_50c_e3_e5": 400,
        "pre_registration_sha256": file_sha256(PREREGISTRATION),
...
def run_formal(cap: int, workers: int, instance_id: str) -> None:
    ...
    order = [item[0] for item in INSTANCES]
    if instance_id not in order:
        raise RuntimeError(f"HALT_UNREGISTERED_INSTANCE:{instance_id}")
    for prior_instance in order[:order.index(instance_id)]:
        completed_prior = [
            json.loads(path.read_text(encoding="utf-8"))
            for path in sorted((HERE / "formal/task_status").glob("*.json"))
            if is_real_artifact(path)
            and path.name.startswith(prior_instance + "__")
        ]
        if len(completed_prior) != 30 or any(
            row.get("status") != "PASS" for row in completed_prior
        ):
            raise RuntimeError(
                f"HALT_INSTANCE_ORDER:{instance_id}:"
                f"{prior_instance}:{len(completed_prior)}/30"
            )
    run_specs(
    ...
    marker = {
        "schema": "resetp.e3-mismatch-restart.instance-stage.v1",
...
def read_formal_statuses() -> list[dict[str, Any]]:
    ...
    expected = len(INSTANCES) * len(ARMS) * len(SEEDS)
    if len(rows) != expected or any(
        row.get("status") != "PASS" for row in rows
    ):
        raise RuntimeError(
            f"HALT_FORMAL_DENOMINATOR:{len(rows)}/{expected}"
        )
    keys = {
...
def _relationship_summary(
    effects: list[dict[str, Any]],
    descriptive_by_instance: dict[str, dict[str, str]],
) -> dict[str, Any]:
    points = [
        (
            float(
                descriptive_by_instance[row["instance_id"]][
                    "registered_nearest_mismatch_pct"
                ]
            ),
            float(row["effect_zone_to_joint_pct"]),
            row["instance_id"],
            "NONZERO_MISMATCH_FORMAL",
        )
        for row in effects
    ]
    points.extend(
    ...
    slope = covariance / x_ss if x_ss else 0.0
    correlation = (
        covariance / math.sqrt(x_ss * y_ss)
        if x_ss and y_ss
        else 0.0
    )
    return {
        "points": [
            {
                "mismatch_rate_pct": x,
                "zone_to_joint_cost_effect_pct": y,
                "instance_id": instance_id,
                "source": source,
            }
            for x, y, instance_id, source in points
        ],
        "zero_mismatch_control_mean_zone_to_joint_pct": mean(
            point[1] for point in points if point[0] == 0.0
        ),
        "nonzero_mismatch_mean_zone_to_joint_pct": mean(
            point[1] for point in points if point[0] > 0.0
        ),
        "linear_slope_effect_pct_per_mismatch_percentage_point": slope,
        "pearson_correlation": correlation,
        "comparison_scope":
            "two sealed 50c/100c zero-mismatch controls plus six "
            "150c/200c nonzero-mismatch formal instances",
    }
...
def aggregate(cap: int, workers: int) -> dict[str, Any]:
    ...
    descriptive_by_instance = {
        row["instance_id"]: row for row in descriptive
    }
    summaries: list[dict[str, Any]] = []
    effects: list[dict[str, Any]] = []
    paired: list[dict[str, Any]] = []
    for instance_id, role in INSTANCES:
        ...
        if any(len(rows) != 10 for rows in by_arm.values()):
            raise RuntimeError(
                f"HALT_INSTANCE_ARM_DENOMINATOR:{instance_id}"
            )
        description = descriptive_by_instance[instance_id]
        summary: dict[str, Any] = {
            "instance_id": instance_id,
            "sample_role": role,
            "customer_count": int(description["customer_count"]),
            "mismatch_customer_count": int(
                description["registered_nearest_mismatch_count"]
            ),
            "mismatch_rate_pct": float(
                description["registered_nearest_mismatch_pct"]
            ),
        }
        for arm in ARMS:
            rows = by_arm[arm]
        ...
        ind = float(summary["IND_Avg_cny"])
        zone = float(summary["ZONE_Avg_cny"])
        joint = float(summary["JOINT_Avg_cny"])
        effect_row = {
            "instance_id": instance_id,
            "mismatch_rate_pct": summary["mismatch_rate_pct"],
            "effect_ind_to_zone_pct": savings_pct(ind, zone),
            "effect_zone_to_joint_pct": savings_pct(zone, joint),
            "effect_ind_to_joint_pct": savings_pct(ind, joint),
        }
        summary.update(effect_row)
        summaries.append(summary)
            ...
            paired.append(
                {
                    ...
                    "ZONE_cost_cny": costs["ZONE"],
                    "JOINT_cost_cny": costs["JOINT"],
                    "effect_ind_to_zone_pct": savings_pct(
                        costs["IND"], costs["ZONE"]
                    ),
                    "effect_zone_to_joint_pct": savings_pct(
                        costs["ZONE"], costs["JOINT"]
                    ),
                    "effect_ind_to_joint_pct": savings_pct(
                        costs["IND"], costs["JOINT"]
                    ),
                }
            )
    atomic_csv(HERE / "formal_summary.csv", summaries)
    ...
    relationship = _relationship_summary(
        effects, descriptive_by_instance
    )
    atomic_json(
        HERE / "mismatch_joint_relationship.json", relationship
    )
    overall = {
        key: mean(float(row[key]) for row in effects)
        for key in (
            "effect_ind_to_zone_pct",
            "effect_zone_to_joint_pct",
            "effect_ind_to_joint_pct",
        )
    }
    decision = {
        ...
        "task_id": TASK_ID,
        "verdict":
            "PASS_THREE_ARM_EFFECTS_REPORTED_WITHOUT_DIRECTION_FILTER",
        "created_at_utc": now_iso(),
        "formal_units_run": len(raw_rows),
        "instances_run": len(INSTANCES),
        "arms": list(ARMS),
        "seeds": list(SEEDS),
        "budget_cap": cap,
        "effects": effects,
        "overall_equal_weight_effects_pct": overall,
        "mismatch_joint_relationship": relationship,
        "no_result_filtering": True,
        "historical_l_main_numbers_used_as_evidence": False,
        "source_lock_id": lock["source_lock_id"],
    }
    ...
    metadata = {
        ...
        "created_at_utc": now_iso(),
        "contract": relative(CONTRACT),
        "pre_registration": relative(PREREGISTRATION),
        "pre_registration_sha256": file_sha256(PREREGISTRATION),
        "instances": [instance for instance, _role in INSTANCES],
        "arms": list(ARMS),
        "seeds": list(SEEDS),
        "formal_units_expected":
            len(INSTANCES) * len(ARMS) * len(SEEDS),
        "formal_units_run": len(raw_rows),
        "budget_cap": cap,
        "budget_semantics": "UPPER_CAP_NOT_QUOTA",
        "workers": workers,
        ...
        "first_fit_equivalence_units": 56,
        "first_fit_equivalence_mismatches": 0,
        "inputs_built": {arm: len(INSTANCES) for arm in ARMS},
...
def render_report(
    ...
    for row in summaries:
        cost_values = [
            row["instance_id"],
            f"{float(row['mismatch_rate_pct']):.3f}%",
        ]
        for arm in ARMS:
            cost_values.extend(
                [
        ...
        cost_rows.append("| " + " | ".join(cost_values) + " |")
        metric_values = [
            row["instance_id"],
            f"{int(row['mismatch_customer_count'])}/"
            f"{int(row['customer_count'])}",
        ]
        for arm in ARMS:
            metric_values.extend(
                [
                    f"{float(row[f'{arm}_distance_m_avg']):.3f}",
                    f"{float(row[f'{arm}_carbon_kg_avg']):.6f}",
                    f"{float(row[f'{arm}_tw_satisfied_avg']):.1f}",
                ]
            )
        metric_rows.append("| " + " | ".join(metric_values) + " |")
        effect_rows.append(
            "| {instance} | {mismatch:.3f}% | {iz:.6f}% | "
            "{zj:.6f}% | {ij:.6f}% |".format(
                instance=row["instance_id"],
                mismatch=float(row["mismatch_rate_pct"]),
                iz=float(row["effect_ind_to_zone_pct"]),
                zj=float(row["effect_zone_to_joint_pct"]),
                ij=float(row["effect_ind_to_joint_pct"]),
            )
        )
    overall = decision["overall_equal_weight_effects_pct"]
    flow_counts: dict[str, int] = {}
    for row in mismatch_rows:
        flow = (
            f"{row['administrative_depot_city']}→"
            f"{row['nearest_depot_city']}"
        )
    ...
    nonzero = float(
        relationship["nonzero_mismatch_mean_zone_to_joint_pct"]
    )
    zero = float(
        relationship[
            "zero_mismatch_control_mean_zone_to_joint_pct"
        ]
    )
    slope = float(
        relationship[
            "linear_slope_effect_pct_per_mismatch_percentage_point"
        ]
    )
    correlation = float(relationship["pearson_correlation"])
    effect_direction = (
        "下降"
        if float(overall["effect_ind_to_joint_pct"]) > 0
        else "上升"
        if float(overall["effect_ind_to_joint_pct"]) < 0
        else "不变"
    )
    return f"""# E3 行政—道路责任错配三臂结构对照

状态：`{decision['verdict']}`。China81 全部 81 个实例、5805 个客户的独立输入复核确认 6 个自然非零错配实例；这 6 个实例全部纳入 IND / ZONE / JOINT 正式实验，每臂种子 1--10，共 {len(INSTANCES) * len(ARMS) * len(SEEDS)} 个正式单元。

## 结论

六个实例行等权汇总后，IND 到 ZONE 的成本变化为 {float(overall['effect_ind_to_zone_pct']):.6f}%，ZONE 到 JOINT 为 {float(overall['effect_zone_to_joint_pct']):.6f}%，IND 到 JOINT 为 {float(overall['effect_ind_to_joint_pct']):.6f}%（正值表示后者成本下降）。因此从行政独立配送转到完全联合配送时，平均成本{effect_direction}。

零错配的 50c/100c 封存对照中，JOINT 相对 ZONE 的成本节省分别为 2.272759% 和 0.693115%，均值 {zero:.6f}%；本轮六个非零错配实例的同一效应均值为 {nonzero:.6f}%。把八个算例按其错配率作描述性线性比较，错配率每提高 1 个百分点，ZONE 到 JOINT 成本效应变化 {slope:.6f} 个百分点，Pearson 相关为 {correlation:.6f}。零错配对照规模为 50c/100c，非零错配组为 150c/200c，本段按算例行比较。

## 81 实例错配复核

非零名单与对抗性审查报告完全一致：`cn-prd-150c-01` 为 3/150（2.000%），`cn-prd-150c-02` 为 2/150（1.333%），`cn-prd-150c-03` 为 4/150（2.667%），`cn-prd-200c-01` 为 7/200（3.500%），`cn-prd-200c-02` 为 6/200（3.000%），`cn-prd-200c-03` 为 2/200（1.000%）。合计 24/5805（0.413437%），最近距离并列为 0。

错配来自行政责任与道路拓扑的边界差异：loader 以客户所在城市映射到同城唯一车场，而有向道路最短距离把部分东莞客户分给广州车场、部分广州客户分给佛山车场。24 个错配客户的流向为 {flow_text}。六个实例由三个 150c 和三个 200c 复本组成，最高错配率实例作为主展示，其余五个覆盖同规模复本与跨规模稳健性。

## 期刊式成本主表

`Gap%=(Avg-Best)/Best×100`。车辆数、时间和实际完整候选评价数均为十次均值；共同 cap {cap} 是上限。

| 算例 | 错配率 | IND Best | IND Avg | IND Gap% | IND 车 | IND 秒 | IND eval | ZONE Best | ZONE Avg | ZONE Gap% | ZONE 车 | ZONE 秒 | ZONE eval | JOINT Best | JOINT Avg | JOINT Gap% | JOINT 车 | JOINT 秒 | JOINT eval |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
{chr(10).join(cost_rows)}

## 距离、碳排和时间窗

| 算例 | 错配客户 | IND 距离m | IND 碳kg | IND 满足TW | ZONE 距离m | ZONE 碳kg | ZONE 满足TW | JOINT 距离m | JOINT 碳kg | JOINT 满足TW |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
{chr(10).join(metric_rows)}

## 三个成本效应

| 算例 | 错配率 | IND→ZONE | ZONE→JOINT | IND→JOINT |
|---|---:|---:|---:|---:|
{chr(10).join(effect_rows)}
| 六算例行等权 | — | {float(overall['effect_ind_to_zone_pct']):.6f}% | {float(overall['effect_zone_to_joint_pct']):.6f}% | {float(overall['effect_ind_to_joint_pct']):.6f}% |

IND 到 ZONE 表示空间组织价值；ZONE 到 JOINT 表示最近车场分区后的剩余协同价值；IND 到 JOINT 表示两者合计。陈雨蝶（2025）表 9 的联合相对分区总成本为 −3.44%、碳排放为 +2.86%、车辆 8→7；陈雨蝶（2023）表 5 的联合相对分区总成本为 −6.09%、距离为 −6.34%、时间为 −5.07%、碳排为 −4.79%。本文采用正值表示成本节省，比较时已统一方向。

## 输入、预算与运行纪律

预注册在任何搜索前锁定全部六个实例、三臂、种子 1--10、实例顺序和预算选择公式。长探针使用最大规模且错配客户最多的 `cn-prd-200c-01`、JOINT、seed 1；正式 cap 为 {cap}。每个实例的 30 个单元全部落盘后再进入下一个。合法不可行和 `FILTERED_HARD_HOME_DEPOT_LOCK` 计入实际消费，真技术错误为 0；并发最多 2 workers，四个数值线程变量固定为 1。

## 正文与存档

正文采用上面的成本主表、机制指标表和三效应表；它们分别承担求解表现、运营结果和研究问题回答。错配客户逐点清单、81 实例全量审计、逐候选 trace、逐解计划、收敛曲线和独立复算证书仅存档。未制作图形：八个算例的错配率—效应关系已由三效应表和两项描述统计完整表达，增加散点图不会增加新的比较维度。本文不设附录。

## 完整性

`raw_runs.csv` 保留全部 {len(INSTANCES) * len(ARMS) * len(SEEDS)} 行；四件套、`pre_registration.json`、本报告、逐解计划、完整 trace、独立复算证书与预算锁构成正式证据面。哈希枚举排除 `._*`、`__pycache__`、`.pytest_cache` 和监控运行态。受保护源文件关闭哈希与预注册一致；旧 E3 目录没有覆盖或删除。
"""
...
def seal(cap: int, workers: int) -> dict[str, Any]:
    ...
    artifact_hashes = {
        "schema": "resetp.e3-mismatch-restart.artifact-hashes.v1",
        "created_at_utc": now_iso(),
        "exclusions": [
            "artifact_hashes.json",
            "done.json",
    ...
    mismatch_rates = {
        row["instance_id"]: float(
            row["registered_nearest_mismatch_pct"]
        )
        for row in descriptive
    }
    overall = decision["overall_equal_weight_effects_pct"]
        ...
        "status": "COMPLETE",
        "nonzero_mismatch_instances_found": len(INSTANCES),
        "instances_used": [
        ...
        "budget_cap": cap,
        "effect_ind_to_zone_pct":
            overall["effect_ind_to_zone_pct"],
        "effect_zone_to_joint_pct":
            overall["effect_zone_to_joint_pct"],
        "effect_ind_to_joint_pct":
            overall["effect_ind_to_joint_pct"],
        "formal_units_run": expected,
...
def main() -> int:
    ...
    if args.command == "prepare":
        ...
        if args.instance_id is None:
            raise RuntimeError("HALT_FORMAL_INSTANCE_ID_REQUIRED")
        run_formal(
            args.complete_eval_budget, args.workers, args.instance_id
        )
