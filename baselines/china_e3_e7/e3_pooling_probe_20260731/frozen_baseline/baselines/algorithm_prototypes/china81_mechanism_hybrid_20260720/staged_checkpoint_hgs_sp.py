"""Two-stage exact-elite HGS with protected and expanded route pools."""
...
class StagedCheckpointHgsSpRun:
    solution: Solution
...
def run_staged_checkpoint_hgs_sp(
    bundle: China81Bundle,
    ...
    *,
    ...
    mip_seconds_per_stage: float = 5.0,
    collect_historical_population_archive: bool = False,
    wallclock_safety_seconds_per_stage: float,
) -> StagedCheckpointHgsSpRun:
    """Run the preregistered protected-base plus deepening candidate."""
    ...
    # Stage 1 exactly preserves the original 5000-iteration search in all
    # three views before any deepening begins.
    for mode in VIEW_ORDER:
        ...
        stage_1 = _run_exact_epoch(
            ...
            wallclock_safety_seconds=(
                wallclock_safety_seconds_per_stage
            ),
    ...
    # Stage 2 deepens each view from only its own complete-model elites.
    for mode in VIEW_ORDER:
        ...
        selected = tuple(ranked[:exact_elites_per_view])
    ...
    stage_2_phase_elapsed = (
        perf_counter() - started - base_phase_elapsed
    )
    ...
    if violations or abs(exact_objective - completion.objective) > 1e-9:
        raise RuntimeError("staged candidate failed final exact check")
    elapsed = perf_counter() - started
    epoch_attempts = sum(
        int(
            stage.stats["complete_candidate_evaluation_attempts"]
        )
        for stage_pair in stages.values()
        for stage in stage_pair
    )
    complete_attempts = epoch_attempts + 4
    return StagedCheckpointHgsSpRun(
        ...
        stats={
            "algorithm": "MV-HGS-SP-STAGED-DEEPENING",
            "seed": int(seed),
            ...
            "complete_candidate_budget_exactly_consumed": (
                complete_attempts == 280
            ),
            "complete_candidate_budget_cap": 280,
            "complete_candidate_budget_within_cap": (
                complete_attempts <= 280
            ),
            ...
            "base_phase_elapsed_seconds": float(
                base_phase_elapsed
            ),
