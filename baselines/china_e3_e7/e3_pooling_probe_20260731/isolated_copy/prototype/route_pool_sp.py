...
def run_hgs_route_pool_recombination(
    ...
    *,
    ...
    pool_fleet: bool = False,
) -> HgsRoutePoolRun:
    """Generate mechanism-diverse HGS elites and recombine their routes.

    ``pool_fleet`` is an e3_pooling_probe_20260731 addition (not present in
    the frozen baseline): when True, ``_solve_set_partitioning`` drops the
    per-depot vehicle-count constraint and enforces only the existing
    global cross-depot cv/ev total constraint (bundle.instance.num_cv /
    num_ev, already present and unchanged).  Per-view HGS candidate
    generation (build_pyvrp_problem) still explores each depot at its own
    local fleet_caps_by_depot allocation, exactly as in the frozen
    baseline; pooling happens at recombination, where the final selection
    can draw more routes from one depot's pool than that depot's own local
    cap by combining routes surfaced across many different HGS candidates.
    Route-to-depot binding (home_depot_id), check.py flow-balance, and
    cost.py pricing are unaffected; the final independent recheck
    (exact_china81_score -> check.py's global-only fleet check) is also
    unaffected and is what actually admits or rejects the pooled result.
    """
