#!/usr/bin/env python3
"""Run the implementable Montoya L1/L2/PL approximations on E5 inputs."""
...
DEFAULT_SEEDS = (1, 2, 3)
DEFAULT_OUTPUT = HERE / "pilot10_montoya_l1_l2_pl_20260801"
        ...
        row.update(
            {
                ...
                "optimization_legal": not violations,
...
def _decision(rows: list[dict[str, Any]], *, expected_unit_count: int) -> dict[str, Any]:
    ...
    return {
        "status": "THREE_IMPLEMENTABLE_MONTOYA_APPROXIMATIONS_COMPLETE",
        "formal_result": False,
        "requested_approximations": ["FS", "L1", "L2", "PL"],
        "executed_approximations": list(APPROXIMATIONS),
        "not_run_approximations": {
            "FS": {
                "status": "NOT_RUN_IMPLEMENTATION_MISMATCH",
                "public_station_soc_cap": FS_PUBLIC_SOC_CAP,
                "reason": (
                    "The solver has no station-only maximum SOC. Its battery "
                    "capacity is shared by depot departure, route feasibility, "
                    "charging, accounting, and validation; setting it to 85% "
                    "would also remove Montoya's full-battery departure."
                ),
            }
        },
...
def _report(metadata: Mapping[str, Any], decision: Mapping[str, Any], rows: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "",
        f"L1 采用首段斜率 13.6/0.62={L1_POWER_KW:.12f} kW；L2 采用首尾连线斜率 16/1.01={L2_POWER_KW:.12f} kW；PL 使用现有 M17 三段曲线。每种近似独立优化，随后固定路线、车型、充电站、开始时刻和充电量，只把占用时间改按 PL 计算。",
        "",
        ...
        "## 全矩阵汇总",
        "",
        "| 近似 | 记录单元 | 自身优化合法 | 固定方案按PL执行合法 | 执行不合法 | 路线不同于PL | 自身成本相对PL均值% | 按PL执行成本相对PL均值% | 执行最小% | 执行最大% |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for approximation in APPROXIMATIONS:
    ...
    *,
    ...
    results.sort(
        key=lambda item: (
            ...
            ("L1", "L2", "PL").index(str(item[0]["approximation"])),
        )
    )
    ...
    metadata = {
        ...
        "literature": [
            {
                ...
                "pages": "PDF 2-4, 13-15",
                "use": "definitions, full-departure assumption, comparison and PL execution recheck",
            },
            {
        ...
        "history_difference": (
            ...
            "pilot independently optimizes L1/L2/PL and keeps every fixed "
