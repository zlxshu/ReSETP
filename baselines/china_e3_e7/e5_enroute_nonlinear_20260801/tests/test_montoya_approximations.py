...
from baselines.china_e3_e7.e5_enroute_nonlinear_20260801.run_montoya_approximations import (
    APPROXIMATIONS,
    FS_PUBLIC_SOC_CAP,
    L1_POWER_KW,
    L2_POWER_KW,
...
def test_only_the_three_honestly_implemented_approximations_are_executed() -> None:
    assert tuple(APPROXIMATIONS) == ("L1", "L2", "PL")
    assert FS_PUBLIC_SOC_CAP == 0.85


def test_l2_and_pl_share_full_charge_time_but_not_intermediate_shape() -> None:
    repo = __import__(
        "baselines.china_e3_e7.e5_enroute_nonlinear_20260801.run_montoya_approximations",
        fromlist=["REPO"],
    ).REPO
