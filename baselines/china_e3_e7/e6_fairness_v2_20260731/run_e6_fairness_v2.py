...
CHECKER = HERE / "check_e6_fairness_v2.py"
TASK_ID = "E6-FAIRNESS-V2-20260731"
CAP = 400
THETA = 1.0
TOLERANCE = 1.0e-9
MAX_WORKERS = 2
...
def persist_candidate_pool(
    ...
    payload = {
        "schema": "resetp.e6-fairness-v2.candidate-pool.v1",
        "task_id": TASK_ID,
...
def run_unit(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if not all(comparisons.values()):
        mismatch = {
            "schema": "resetp.e6-fairness-v2.bitwise-mismatch.v1",
            "task_id": TASK_ID,
            "created_at_utc": now_iso(),
            "instance_id": instance_id,
            "seed": seed,
    ...
    status = {
        "schema": "resetp.e6-fairness-v2.unit-status.v1",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
        "status": "PASS",
        "instance_id": instance_id,
...
def finalize() -> dict[str, Any]:
    ...
    for instance_id, sample_role in INSTANCES:
        ...
        for seed in SEEDS:
            ...
            selected_payload = {
                "schema":
                    "resetp.e6-fairness-v2.selected-f-solution.v1",
                "task_id": TASK_ID,
                "instance_id": instance_id,
                "sample_role": sample_role,
                "seed": seed,
    ...
    metadata = {
        "schema": "resetp.e6-fairness-v2.metadata.v1",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
    ...
    decision = {
        "schema": "resetp.e6-fairness-v2.decision.v1",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
        "status": "PASS_PENDING_INDEPENDENT_VERIFICATION",
...
def artifact_manifest() -> dict[str, Any]:
    ...
    for path in sorted(HERE.rglob("*")):
        if (
            ...
            or any(
                ...
                or part.startswith("monitor_runtime")
    ...
    payload = {
        "schema": "resetp.e6-fairness-v2.artifact-hashes.v1",
        "created_at_utc": now_iso(),
        "files": files,
        "exclusions": [
            "._*",
...
def seal() -> dict[str, Any]:
    ...
    overall = decision
        ...
        "schema": "resetp.e6-fairness-v2.done.v1",
        "task_id": TASK_ID,
        "created_at_utc": now_iso(),
        "status": "COMPLETE",
        "joint_rerun_bitwise_identical": True,
...
def write_halt_package(error: BaseException) -> None:
    ...
    metadata = {
        ...
        "candidate_pool_persisted": bool(completed),
    ...
    (HERE / "report.md").write_text(
        ...
        f"状态：`{status}`。\n\n"
        f"错误：`{message}`\n\n"
        f"已完成 JOINT 单元：{len(completed)}/20；发现逐位不一致后停止，"
        "未调参、未修饰、未继续计算 F。\n",
        encoding="utf-8",
    )
    manifest = artifact_manifest()
