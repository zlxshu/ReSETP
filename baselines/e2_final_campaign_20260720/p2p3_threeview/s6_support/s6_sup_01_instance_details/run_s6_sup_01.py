#!/usr/bin/env python3
"""S6-SUP-01: source-bound export of the 50-customer representative instance."""
...
def build_rows() -> tuple[list[dict[str, str]], list[dict[str, str]], dict[str, Any]]:
    ...
    selected_nodes = [
        ...
        if row["node_type"].strip().lower() in {"customer", "depot"}
    ]
    customers = sorted(
    ...
    if len(customers) != 50 or len(depots) != 2 or len(selected_nodes) != 52:
        raise RuntimeError(
            f"source selection mismatch: customers={len(customers)}, "
            f"depots={len(depots)}, selected={len(selected_nodes)}"
        )
...
def write_tex(rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        lines.append(
            " & ".join(row[field] for field in DETAIL_FIELDS) + r" \\"
        )
...
def main() -> int:
    ...
    raw_ledger = [
        {
            ...
            "selected_row_count": counts["selected_customer_rows"] + counts["selected_depot_rows"],
        },
        {
    ...
    metadata = {
        "schema_version": "resetp.e2.s6-sup-01-instance-details.v1",
    ...
    report = [
        ...
        "",
        f"从冻结节点源导出 50 个客户和 2 个车场，共 {len(detail_rows)} 行；客户编号为 1--50，车场按城市排序为广州 51、深圳 52。",
        "",
