...
def output_path(filename: str) -> Path:
    """Return a revisioned output path while preserving the original v5 names."""
    if OUTPUT_VERSION == "v5":
        return OUT / filename
    stem, suffix = filename.rsplit(".", 1)
    if stem in {"metadata", "decision", "report", "raw_runs", "artifact_hashes"}:
        stem = f"{stem}_{OUTPUT_VERSION}"
    else:
        stem = stem.replace("_v5", f"_{OUTPUT_VERSION}")
...
def copy_data_inputs() -> dict[str, str]:
    shutil.copyfile(CURVE_DISPLAY_V4, output_path("figure4_convergence_v5.csv"))
    shutil.copyfile(CARBON_DISPLAY_V4, output_path("figure3_carbon_profile_v5.csv"))
    shutil.copyfile(APPROVED_CURVE, output_path("figure4_curve_data_approved_v5.csv"))
    return {
...
def plot_figure3() -> dict[str, Any]:
    ...
    ax.legend(
        ...
        edgecolor="#777777", framealpha=0.88, borderpad=0.12,
        handlelength=1.35, handletextpad=0.28, labelspacing=0.08,
        fontsize=5.8,
    )
    fig.subplots_adjust(left=0.16, right=0.985, bottom=0.18, top=0.975)
    fig.savefig(output_path("figure3_carbon_profile_v5.pdf"), bbox_inches="tight")
    fig.savefig(output_path("figure3_carbon_profile_v5.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_figure4() -> dict[str, Any]:
    ...
    fig, (ax_top, ax_bottom) = plt.subplots(
        ...
        gridspec_kw={"height_ratios": [1.25, 3], "hspace": 0.045},
    )
    ...
    # Keep the early descent visible in the upper panel.  The former 2580
    # lower bound left only the common initial point and looked like a broken
    # renderer, especially after PDF inclusion at paper width.
    ax_top.set_ylim(2510.0, 2610.0)
    ax_bottom.set_ylim(2350.0, 2500.0)
    ax_top.set_yticks([2520, 2560, 2600])
    ...
    # Put the legend in the empty upper-right area of the lower panel.  The
    # upper panel contains the informative early descent and should remain
    # unobstructed.
    ax_bottom.legend(
        loc="upper right", frameon=True, fancybox=False,
        edgecolor="#777777", framealpha=0.88, borderpad=0.12,
    ...
    fig.savefig(output_path("figure4_convergence_v5.pdf"), bbox_inches="tight")
    fig.savefig(output_path("figure4_convergence_v5.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    return {
        "rows": len(rows),
        "font": font_name,
        "series": list(FIG4_ORDER),
        "broken_y_axis": {"lower": [2350, 2500], "upper": [2510, 2610]},
...
def build_raw_ledger(source_hashes: dict[str, str]) -> None:
    ...
    write_csv(
        output_path("raw_runs.csv"), output_rows,
...
def artifact_files() -> list[Path]:
    return sorted(
        ...
        if path.is_file()
        and path.name != output_path("artifact_hashes.json").name
...
def main() -> int:
    ...
    output_csv_hashes = {
        output_path("figure3_carbon_profile_v5.csv").name: sha256(output_path("figure3_carbon_profile_v5.csv")),
        output_path("figure4_convergence_v5.csv").name: sha256(output_path("figure4_convergence_v5.csv")),
        output_path("figure4_curve_data_approved_v5.csv").name: sha256(output_path("figure4_curve_data_approved_v5.csv")),
    }
    metadata = {
        "schema_version": f"resetp.e2.s6-sup-02-figure-{OUTPUT_VERSION}.v1",
        ...
        "display": {
            ...
            "figure4_broken_y_axis": {"lower": [2350, 2500], "upper": [2510, 2610]},
            "grid": False,
    ...
    decision = {
        ...
        "decision": f"PASS_S6_SUP_02_FIGURES_{OUTPUT_VERSION.upper()}",
        "approval_register_id": APPROVAL_REGISTER_ID,
    ...
    write_json(output_path("decision.json"), decision)
    report = [
        f"# S6-SUP-02 图3/图4中文化 {OUTPUT_VERSION}",
        "",
        f"机器判定：`PASS_S6_SUP_02_FIGURES_{OUTPUT_VERSION.upper()}`。",
        "",
        f"图3和图4均只重生成呈现层。图3读取 S5 v4 的 144 行碳强度 CSV，图4读取经 `S3-TRAJ-CURVE-DEF-001` 批准并已封存的 29 行选定轨迹 CSV；两个 {OUTPUT_VERSION} CSV 与相应 v4 CSV 字节一致。",
        "",
        "图4使用断轴：下段 2350--2500，上段 2510--2610，图中用断轴标记连接；上段保留早期下降轨迹，避免只显示共同初始点。曲线数据、预注册种子和单调 best-so-far 口径不变。旧 `figure4_curve_data_v2.csv` 仅作为历史文件记录哈希，没有被重新用于当前图，以免越过 S3-TRAJ 的最新批准口径。",
        "",
        "图3轴标签为“时刻(h)”和“碳强度(gCO2/kWh)”，图例为北京/广东/重庆；图4轴标签为“时间(min)”和“成本(元)”，图例为 HGS-F/HGS-E/HGS-M/MV-HGS-SP。图例缩小并降低遮挡；图4图例移至下段右上空白区。两图均为 4.30×2.80 英寸、细线异线型、无网格，并输出矢量 PDF 与 300 dpi PNG。",
        "",
        "主 TeX、封存数据、评价器和保护文件均未修改。",
    ]
    output_path("report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
    write_json(
        output_path("artifact_hashes.json"),
        {
    ...
    print(f"PASS_S6_SUP_02_FIGURES_{OUTPUT_VERSION.upper()}")
    return 0
