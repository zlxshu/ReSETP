#!/usr/bin/env python3
"""Wait for protected E2 v7, then run G0, worker probe, and G1."""
...
REGISTRATION = PACKAGE / "g0_g1_release_chain_registration_v2.json"
OUT = PACKAGE / "g0_g1_release_chain_v2"
...
PROBE_RUNNER = (
    ...
    "run_g1_six_worker_resource_probe.py"
)
G1_RUNNER = PACKAGE / "run_g1_micro_gate.py"
G0_GATE = PACKAGE / "g0_real_bundle_gate_v1"
PROBE_GATE = PACKAGE / "g1_six_worker_resource_probe_v1"
G1_GATE = PACKAGE / "g1_micro_gate_v3"
FORMAL_CAMPAIGN = (
    REPO
...
def verify_registration() -> dict[str, Any]:
    ...
    if (
        ...
        != "resetp.genuine-hybrid-g0-g1-release-chain-registration.v2"
    ):
...
def update_progress(*, stage: str, status: str, detail: str) -> None:
    write_json(
        PROGRESS,
        {
            "schema": "resetp.genuine-hybrid-g0-g1-progress.v2",
...
def terminal_write(
    ...
    *,
    ...
    decision_payload = {
        "schema": "resetp.genuine-hybrid-g0-g1-release-chain-decision.v2",
        "decision": decision,
        ...
        "worker_probe_decision": next(
            (row["decision"] for row in rows if row["stage"] == "worker_probe"),
            "NOT_RUN",
        ),
        "selected_g1_workers": selected_workers,
        "g1_decision": next(
        ...
        "claim_boundary": (
            ...
            "the probe selects parallel task count without search; G1 is a "
            "three-development-instance micro gate and cannot support formal "
            "China81, BKS, or SOTA claims."
        ),
    }
    ...
    write_json(
        OUT / "metadata.json",
        {
            "schema": "resetp.genuine-hybrid-g0-g1-release-chain-metadata.v2",
    ...
    report = "\n".join(
        [
            "# Genuine hybrid G0--worker-probe--G1 release chain",
            "",
            f"Decision: `{decision}`.",
            "",
            *[
            ...
            "",
            f"Selected G1 workers: `{selected_workers}`.",
            "",
            (
                "The chain did not change algorithms, budgets, inputs, "
                "evaluators, random seeds, or acceptance gates."
            ),
            "",
        ]
    )
    ...
    write_json(
        OUT / "done.json",
        {
            "schema": "resetp.genuine-hybrid-g0-g1-done.v2",
            "decision": decision,
...
def execute(poll_seconds: float) -> int:
    ...
    if not g0_pass:
        terminal_write(
            registration,
            rows,
            decision="STOP_G0_REAL_BUNDLE",
            selected_workers=None,
        )
        return 0

    probe_decision, probe_returncode = run_stage(
        stage="worker_probe",
        command=[str(PYTHON), str(PROBE_RUNNER)],
        gate=PROBE_GATE,
        decision_field="decision",
    )
    worker_map = registration["worker_selection"]
    if probe_decision == str(worker_map["pass_decision"]):
        selected_workers = int(worker_map["workers_if_probe_passes"])
    elif probe_decision == str(worker_map["fallback_decision"]):
        selected_workers = int(worker_map["workers_if_probe_falls_back"])
    else:
        rows.append(
            {
                "stage": "worker_probe",
                ...
                "detail": f"unexpected decision;returncode={probe_returncode}",
                "workers": "",
            }
        )
        terminal_write(
            registration,
            rows,
            decision="STOP_G1_WORKER_PROBE_INVALID",
            selected_workers=None,
        )
        return 0
    rows.append(
        {
            ...
            "status": "PASS",
            "decision": probe_decision,
            "detail": f"returncode={probe_returncode}",
        ...
        json.dumps(
            {
                ...
                "worker_selection": registration["worker_selection"],
