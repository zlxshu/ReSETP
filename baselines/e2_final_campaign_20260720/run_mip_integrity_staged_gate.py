#!/usr/bin/env python3
"""Blind gate for preserving checked MIP incumbents with a 30-second cap."""
...
OUT = (
    ...
    "mip_integrity_staged_gate"
)
PREREGISTRATION = (
    OUT.parent / "mip_integrity_staged_preregistration.json"
)
PANEL = (
    ("cn-cy-150c-02-V2-LOCATIONS", 16),
    ("cn-jjj-150c-02-V2-LOCATIONS", 16),
    ("cn-prd-150c-02-V2-LOCATIONS", 16),
    ("cn-cy-200c-01-V2-LOCATIONS", 16),
    ("cn-jjj-200c-01-V2-LOCATIONS", 16),
    ("cn-prd-200c-01-V2-LOCATIONS", 16),
)
MAXIMUM_BUDGET = 280
...
def main() -> int:
    ...
    verdict = (
        "PASS_MIP_INTEGRITY_STAGED_DEVELOPMENT_GATE"
        if passed
        else "HALT_MIP_INTEGRITY_STAGED_DEVELOPMENT_GATE"
    )
    decision = {
        "schema": (
            "resetp.mip-integrity-staged-development.decision.v1"
        ),
        "verdict": verdict,
    ...
    development.write_json(
        ...
        {
            "schema": (
                "resetp.mip-integrity-staged-development.metadata.v1"
            ),
    ...
    (OUT / "report.md").write_text(
        "# MIP-integrity staged development gate\n\n"
        f"Decision: `{verdict}`.\n\n"
        f"The fresh seed-16 panel completed {len(rows)}/6 tasks with "
        f"{len(errors)} errors and {expanded_mip_incumbents}/6 checked "
        "expanded-pool MIP incumbents. It produced "
        ...
        "gate. Checked MIP incumbents retain their exact routes, vehicle "
        "types, charging actions, and depot assignments.\n",
        encoding="utf-8",
    )
    artifacts = {
