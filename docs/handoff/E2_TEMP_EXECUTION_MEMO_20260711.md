...
旧270行已经完成，但用户决定把E2最终表补成九个自有算例×九算法×每组十次×4000评价，共810行。旧主表只复用225行，立即充电45行转E3消融，新增缺口585行。先跑27项200评价接线门，再跑18项4000评价代表门；两关都正确才启动可恢复长跑。默认2 workers，30--60分钟低频监控。

E2不设单独完整模型复核层，不开公平，跨场费0，280 kWh主场景、80 kWh备用。自有算例不套BKS/AVG/Gap%标准表；标准算例、公开BKS/AVG/Gap%和未经改动Goeke原始算法留到E2后补实验。E2彻底收口后立即准备E3。唯一合同：`docs/handoff/e1_e7_submission_contract_decision_20260711.md`；以下旧270行安排只作历史。

统计必须落成主算法对八基线的Wilcoxon符号秩检验，报告原始p值和Holm校正p值。代表门18行通过后可进入正式矩阵，所以实际剩余补跑567项。E2长跑期间只做客户归属、跨场费和线性碳配额的只读文献核对，不开其他实验；Goeke公开标准算例补实验于7月24日前评估可行性，做不完明确降级。

代表门已通过：18/18满4000评价、零违规、当前检查器复算和哈希齐全。两项IWD未超过共同起点，但其台账显示各自完成3810次构造和190次局部搜索；这是真实的“跑了但没赢”，不是入口没接上。门禁已改为“有自身改进，或有非空自身操作台账”即可证明真实活动；不改求解器、预算、成本、物理约束或保存解。18行可复用，正式仍缺567项。
...
8. 写作边界：这是“分阶段混合搜索 + 固定路线充电重调度”的组合式创新，不是已经证明的全新ALNS。E2成绩只证明性能；后续还须做最近文献对照和四臂消融，才可判断算法新颖性强弱。禁止把独立代码包或排名第一直接写成“独创算法”。
