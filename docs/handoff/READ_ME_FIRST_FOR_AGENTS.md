...
运行并发遵循项目实验手册：安全时，8逻辑线程机器尽量使用6--8个独立进程，16逻辑线程机器按比例使用12--16个；任务数不足时只启动真实任务数，内存、温度、磁盘写冲突或总吞吐恶化时按证据降级。所有运行通过`scripts/run_with_status_hook.py`记录状态；短跑/中程/长跑的检查间隔分别为5/10/15分钟。
