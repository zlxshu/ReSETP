...
## 2. 当前必须由 user 拍板的分叉
不要继续自动进入 E2/T3。先在以下三条中选一个主叙事,再写下一条 Codex 执行提示词。

1. **旧文献锚主场景**:接受 `80kWh` 作为 Goeke/Chen 文献 benchmark 主场景,优点是 09k full gate 下严格 balanced,缺点是它是旧/文献值,很难作为"现代跨城配送电动卡车"参数。若选它,要把 09i/09j 的 280kWh 默认改回或拆成 scenario,并在论文中诚实承认现代电池情景另作敏感性。
2. **100kWh 边界候选**:把 Isuzu NRR EV 的 `100kWh` 作为证据边界候选继续确认,优点是现代中型车证据、处在过渡边界;缺点是 09k full gate 中 winner 多数已 EV-heavy（30/51）,不能直接说稳定 balanced。若选它,下一步应只做更强预算/seeds1-5 + composition robustness + 成本分解,通过才考虑改默认。
3. **运营约束退路(我认为最科学,但要 user 拍)**:承认现代中型/配送车电池下跨城算例经济上趋向 EV 主导,混合车队应来自现实运营约束:场站充电容量/公共桩稀缺、EV 购置资本或车队数量上限、部分长路线 EV eligibility、充电窗口/峰谷时段限制。若选它,下一步不是调电池,而是做 10k/09l 式运营约束证据矩阵 + 最小语义变更设计 + replay/reopt gate。
...
- **若选 100kWh 仍 EV-heavy**:不要继续向下微调电池找刀刃,转运营约束退路。
- **若选运营约束退路**:先做证据矩阵和最小语义设计,不要一上来改 `cost.py/check.py/evaluation.py`;任何新增约束必须有现实解释、可关开关、旧语义可复现。
...
## 一句话给 Codex
09k 已证明:80kWh 旧锚平衡,280kWh 现代锚 EV 主导,100kWh 是边界但未稳。**不要自驱进入 E2/T3;先让 user 在旧文献锚、100kWh 边界确认、运营约束退路三者中拍板**。全程诚实+克制+用 fable5 思考方式约束自己,跑不出就 HALT、绝不注水。
