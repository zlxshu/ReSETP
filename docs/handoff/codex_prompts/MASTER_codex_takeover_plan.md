...
## 一句话给 Codex
09l/09m/09n/09o 已证明:真实来源电池值单参数不能让非 80kWh 候选跨所有梯度规模稳定维持 20%-80% 实践混合带;unbounded fleet 是真实线索,但原始 Goeke/ReSETP 车辆数 cap 用 SearchPolicy 壳诊断后没有给出可正式化方案;最新 09o 小试探显示成功 280kWh free-mixed 行 100% 靠车场充电、公共充电为 0。**不要自驱进入 E2/T3,也不要继续调无来源电池值或直接硬套车辆数上限;下一步按 `09p_depot_charger_capacity_evidence_gate.md` 做车场充电桩容量/充电资本预算证据矩阵+最小语义设计,再由 user 拍板是否实现。** 全程诚实+克制+用 fable5 思考方式约束自己,跑不出就 HALT、绝不注水。
