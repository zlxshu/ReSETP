...
- **09l/09m/09n/09o 已给出下一步方向**:真实来源电池容量单参数不足,原始车辆数量上限的 SearchPolicy 诊断壳也不足以直接支撑跨规模 20%-80% 实践混合带。09o 又显示 280kWh 成功小样本不靠公共快充,而靠车场补电。因此不要继续微调无来源电池,也不要直接把 Goeke 车数写成硬约束。下一步应进入更真实的运营约束设计,优先查车场充电桩容量/充电资本预算,其次查公共桩可用性/路线 eligibility/EV 资本预算组合；若继续研究 fleet count,必须先正式定义车辆复用/车次数学语义。任何运营约束都要先有证据矩阵和最小语义设计,不能直接改 `cost.py/check.py/evaluation.py`。
...
- **09l 已确认真实来源候选均不跨规模通过;09m/09n/09o 确认 operation 是真线索但尚未形成可正式化约束**:不要继续向下微调电池找刀刃;后续主线进入运营结构诊断。
...
## 一句话给 Codex
09l/09m/09n/09o 已证明:真实来源电池值单参数不能让非 80kWh 候选跨所有梯度规模稳定维持 20%-80% 实践混合带;unbounded fleet 是真实线索,但原始 Goeke/ReSETP 车辆数 cap 用 SearchPolicy 壳诊断后没有给出可正式化方案;09o 小试探显示成功 280kWh free-mixed 行 100% 靠车场充电、公共充电为 0。user 已纠偏:不能只测 280 或 75-200,必须把真实电池梯度和运营约束放进 10-200 全规模矩阵。**不要自驱进入 E2/T3,也不要继续调无来源电池值或直接硬套车辆数上限;下一步按 `09q_source_bound_operational_mix_map.md` 跑综合可恢复矩阵,跑不满就 HALT,不能把 smoke 或 diagnostic-only pass 写成论文结论。** 全程诚实+克制+用 fable5 思考方式约束自己,跑不出就 HALT、绝不注水。
