...
状态：`P26 IMPLEMENTED AND TECHNICALLY VERIFIED — FORMAL CORE PENDING`
...
`FACT`：用户批准后，静态侧已不再只返回一份确定性充电账本。`duty_hgs/charging.py:182-348` 对每趟联合枚举车场预充、公共站路径和结构电量，再按同一实体车的多趟组合重建完整 SOC 与时钟。公共站路径不会先被路线代理合并；相关入口在 `solver/src/setp_solver/algorithms/resetp_alns/support/charging.py:384-499`。

`FACT`：结构电量现包含输入曲线自己的全部内部拐点。`support/charging.py:96-107` 从 `charging_soc_breakpoints` 生成 `curve_knee_1`、`curve_knee_2` 等候选名，`duty_hgs/charging.py:193-207` 自动并入每趟候选。算法没有写死 80%、90% 或 93%；正式曲线仍由用户另行决定。

`FACT`：静态时刻搜索已同时处理两类信息。单车内部的价碳时段与非线性相位边界继续由 `_timing_candidates()` 枚举；`duty_hgs/charging.py:355-477` 另把同一站点其他车辆充电结束时刻作为共享充电容量释放候选，逐份完整重放后只保留真实可行账本。该入口直接读取当前完整个体，未经过 `ChargingRepairCache`，因此没有把依赖全局会话的结果误放进原单车缓存。

`FACT`：动态侧不是空动作。`duty_hgs/dynamic.py:260-271` 对每个未来路线候选无条件调用 `reschedule_dynamic_charging()`；后者在 `dynamic_multitrip_schedule.py:546-687` 内，只在继承状态后的合法车场充电窗口中比较真实价碳时段并同步证书。站点、充电量、车辆、客户、出发时刻和已执行历史仍不改。

`FACT`：公平同样存在真实作用通道。跨车场客户移动、客户交换和整趟交换会改变实际服务车场；完整利润与参与缺口进入罚后候选比较。因此公平能够改变客户归属，只是当前没有额外按“哪个车场正在吃亏”缩减或偏置候选集合。Claude Opus 5 在最后复核中撤回了“公平没有抓手”和“动态时变碳没有抓手”两项误判。

`FACT`：珠三角 150 客户当前登记解的一辆电动车实测生成 153 份不同整日充电候选，其中 27 份通过全模型，完整枚举与评价约 1.97 秒；进程最大常驻内存约 102 MB。该车只有一趟、当前候选未使用公共站，因此这只回答当前分支量级，不能代替多趟和公共站正式规模测量。

`CORRECTION`：施工中第一次分支计数脚本调用了不存在的 `DutyIndividual.replace_duties()`，以 `AttributeError` 结束；改用不可变数据结构的 `dataclasses.replace()` 后才得到上面的数字。第一次“其他车辆会话结束时刻”测试又把 123 秒放进一条本来没有时间余量的公共站路线，正确地没有产生候选；改在有真实余量的两趟路线验证后通过。这两次失败均为检验脚本或场景选择问题，没有隐藏。

`FACT`：种群侧只做不改变搜索含义的机械提速。`duty_hgs/population.py:88-104` 在候选入群时固定保存指纹和边集合，`:297-349` 与 `:467-482` 在同次适应度和清退计算中复用两两距离；交叉源码注释也已更正为“随机连续整车块交换”，不再冒充 SREX。

`FACT`：扩大后的联动测试把原型全套测试与多趟排班、动态排班、非线性充电、利润评价和公开路线接口放在同一轮执行，最终为 220 项全部通过，耗时 138.43 秒。它证明当前已写入的接线和回归断言相容，不替代正式性能与效应实验。

`BOUNDARY`：本轮仍没有决定正式非线性曲线、正式算法核心、正式墙钟细则、正式单干利润基准或统一算例，也没有启动三大正式实验。使固定起点的充电结束精确贴住价碳边界的额外电量，只对途中公共站或零宽起点有新增自由度，暂未施工并在此保留；没有把它悄悄写成已完成。

## 十、原施工选择（历史记录）

`USER DECISION（2026-08-08）`：用户已批准选项一施工。该批准只覆盖本文件所述整日多候选充电搜索，不代表正式曲线、正式算例、公开算法主张或正式实验已经获选。
...
## 十一、AGENTS.md 第八节九条自检

1. 每个 `FACT` 是否都指到了文件行号 / 产物哈希 / 论文页码？——列出无法指到出处的那几条。

实际答案：代码和运行事实均给出当前源码行号、保存产物或本轮终端复算；Froger 与 Wang 的方法事实给出作者稿页码。公开算法获胜、三大实验效应和最终算例仍没有数据，未写成事实。

2. 有没有把自己的建议或担忧写成“已决”或“状态”？——逐个复查你写的每张表。

实际答案：没有。P26 只在用户 2026-08-08 明确“批准施工”后改成已决；DCREX、MDM、正式曲线、停机细则、Pi0 和算例仍是待决，顾问意见没有写成用户状态。

3. 改动范围有没有超出任务文本？——超出的每一处单独列出并说明理由。

实际答案：本轮按 P26 批准范围修改整日充电候选、动态安全择时接线、相关时钟修复、测试和技术 runner；另修种群重复计算与交叉注释，两项均不改变研究含义。没有改模型、论文正文、正式参数或用户未批准的算法核心。
...
实际答案：P26 已由用户选定，不再重复提问。下一批待决已压成算法核心、墙钟含义、Pi0 三组选择，统一在对话中一次提交。
...
实际答案：保留了公开 PR17A 60 秒增强臂比母体高 27 成本、分支计数脚本 `AttributeError`、无余量测试未生成 123 秒候选，以及正式公开母核、三大实验效果仍未知。扩大测试随后正常结束为 220 项全部通过，没有把等待过程误报成挂死，也没有把 60 秒单种子技术信号包装成算法性能结论。

8. 四件套齐了吗？——逐个报文件名（无实验则写“本任务不产生实验包”）。

实际答案：动态系统接线包和公开 PR17A 技术包均含 `metadata.json`、`raw_runs.csv`、`decision.json`、`artifact_hashes.json`、`report.md`；公开包另有 `solutions.json`，动态包另有 `best_solution.json`。分支计数只是终端诊断，不冒充实验包。

9. `HANDOFF.md` 变更日志和 `docs/handoff/memory/` 同步了吗？——报写入位置。

实际答案：已在同轮同步 `HANDOFF.md` 文末、`docs/handoff/memory/MEMORY.md` 和 `pending_decisions.md`；P26 保留用户批准状态，尚未由用户选择的算法核心、墙钟细则和 Pi0 继续登记为待决。
