...
- [4.4 节经历与 4.4.3 失败材料（给 Codex）](../section_44_history_for_codex_20260910.md) — 09-10 凌晨:五版尝试、四臂两格全数据、结构性事实(成本最优器打不过排放最小化;充电时刻上限16.6kg;平价刀刃;搜索偏向)与五个问题
- [固定方案四规则调时诊断（2026-09-10）](../fixed_plan_retime_diagnosis_20260910.md) — 4.4.2 真因：首趟前窗口价格锁（P=0.2 要 2 元/kg 才去净时段）＋班次窗口锁 17–19 kg；碳信息 P=1.0 起付费减排 14 kg（0.5 元/kg）；午谷解决午休/趟间；臂间排放差大头是车队效应，午谷×1.0 车队刀刃使四臂排放名次成噪声；工具 `solver/reports/fixed_plan_retime_20260910/`
- [4.4.2 原因诊断验收与更正（2026-09-08）](../section44_reason_validation_20260908.md) — 120份/480重排与严格固定时钟口径核验；4份失败确认为跨趟贪心调时无回退；`carbon_min`、窗口/价格差、0.76、路径分解、有限解池和共同候选的证据边界；未形成新用户决定
- [4.4 原因与方案定稿（2026-09-10）](../section44_cause_and_solution_20260910.md) — 原因＝首趟前窗口冲突 4.09 元/kg vs 碳价 0.2＋燃油 64%/车队平价；方案＝午谷解午休趟间、碳价≥0.5 解首趟前（+7 元/−14 kg）、≈1.0 推车队；撤"回场提前"通道；待批固定车队 2/4 四臂 40 跑
- [代理电价失真诊断与开关（2026-09-10）](../proxy_pricing_diagnosis_20260910.md) — 只看碳内生批电动车最多的来源：各批首趟窗口开启时刻不一致（网格批自估 15:49/16:20 vs 09-09 批常数 15:47，午谷 15:30→16:00 碳强度陡升）＋单行计价不查充电时长；开关 `--proxy-feasible-slots` 默认关逐位不变；离线探针 `solver/reports/proxy_feasible_probe_20260910/`；唯一短跑 `charging_arrangements_midday_P1.0_proxyfix_probe_20260910/`
- [只看碳两单条件正式批（2026-09-10）](../../../solver/reports/carbon_min_two_conditions_20260910/README.md) — 表 11 四格数据与 Welch p；4.4 主角换成碳强度引导有序充电后的全部数字出处
- [P177 同一企业多中心身份更正](../../paper_gci_dmm_vrp_20260804/pending_decisions.md) — 2026-09-11：用户明确企业为同一个企业，研究对象为多个配送中心（车场）；取代P65-1多企业身份，Shapley与成本分摊改按中心解释，论文数值不变。
...
- **2026-09-12｜FACT｜算法章图文定稿**：图1重新布局，步骤补清候选解、完整评价、接受条件与反馈；当前证据见CURRENT_PROJECT_CONTEXT.md当日“算法章图文定稿”条，正文docs/paper_v2/paper_main.tex，图源generated_figures/figure_1_algorithm_flow.tex。用户授权Codex定稿，仅图文，未改算法；PDF26页，算法章第9–10页，整篇编译仍退出1。
...
- **FACT（2026-09-12，算法步骤11→9获批压缩）**：当前事实见CURRENT_PROJECT_CONTEXT.md末条；步骤中文字符减少23.5%，算法章整体减少24.8%，论文26→25页；保留算法规则，仅补全充电准则与步骤引用。源稿docs/paper_v2/paper_main.tex，备份及日志/tmp/resetp-algorithm-condense-20260912/。

- **FACT（2026-09-12，3.1开头精简）**：见CURRENT_PROJECT_CONTEXT.md末条。3.1仅留问题—方法概述；实现信息移至3.2，PDF25页。

- **FACT（2026-09-12）**：3.2已取消开场段，必要定义归入相应步骤；当前事实见CURRENT_PROJECT_CONTEXT.md末条，PDF仍25页。

- **FACT（2026-09-12，算法章内容归属）**：3.1总体框架与评价区别，3.2直接列操作步骤，4.1.1记软件实现；PDF25页。详见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，陈雨蝶全文步骤写法对照）**：步骤1—5补清编码/选择/交叉/修复的真实操作与参数，步骤文字净增111字，仍25页；原文§3.2正文8—9页，代码证据及验收见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12）**：步骤4以跨中心拆分操作直接描述，保留来源引用，去实现过程措辞；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，软件说明位置）**：PyVRP说明改为3.1框架处脚注，明确种群管理/二元锦标赛/路径交换用途，4.1.1移除旧句；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，MTC-HGS正式定义）**：全称与缩写首次统一在3.1出现，框架措辞保留HGS/PyVRP引用，删除实现脚注，PDF25页；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，连续引用清理）**：3处成组引用改为单篇，活跃TeX无连续引用，44条均仍有独立引用；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，HGS名称）**：3.1正式定义Hybrid Genetic Search/HGS，官方仓库核名；当前事实见CURRENT_PROJECT_CONTEXT.md末条，PDF25页。

- **FACT（2026-09-12）**：MTC-HGS中文全称补全多趟次配送/车型置换/碳强度引导充电限定，与HGS基础算法区分；详见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，3.1流程概括）**：3.1只概括搜索/改进/反馈/动态衔接，成本规则归入步骤2；详见CURRENT_PROJECT_CONTEXT.md末条，PDF25页。

- **FACT（2026-09-12，3.1方法设计理由）**：3.1依陈雨蝶母版交代问题约束、方法名称和各组成方法作用；仅改概述，详见CURRENT_PROJECT_CONTEXT.md末条，PDF25页。

- **FACT（2026-09-12，3.1整合定稿）**：用户认可的因素—模型全称—求解难点—算法全称及应对方法已合并进3.1；证据见CURRENT_PROJECT_CONTEXT.md末条，PDF25页。

- **FACT（2026-09-12，痛点与能力）**：3.1补强组合规模/充电耦合/执行状态限制及相应求解机制，仅改本段，PDF25页；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，3.1再压缩）**：按用户要求仅留核心痛点/能力，段落中文减少37%，PDF仍25页；详见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12，3.1用语）**：按母版采用多重约束/全局搜索/局部优化/当前最优解直接表述，仅改3.1，PDF25页；见CURRENT_PROJECT_CONTEXT.md末条。

- **FACT（2026-09-12）**：3.1按批注用“相互影响”替换“相互耦合”，PDF25页；见CURRENT_PROJECT_CONTEXT.md末条。
...
2026-09-12｜中心协作与成本分摊前提：同一企业下各中心实行独立核算并作为协作参与者；分摊影响其维持协作的意愿，可能退出的是联合配送协作而非企业。论文统一使用“中心”，证据入口见`docs/paper_v2/paper_main.tex`模型假设、`pending_decisions.md`模型身份条目及`CURRENT_PROJECT_CONTEXT.md`当日记录。
2026-09-12｜更正上一条：删除无文献支持的“独立核算”和同一企业内部中心退出机制。原始文献的准确主体是成员企业退出联盟，或中心作为联盟成员退出协作；未证明同一企业下中心具有相同退出机制。当前身份冲突见`pending_decisions.md`与`CURRENT_PROJECT_CONTEXT.md`当日更正。
2026-09-12｜全文用语复查：删除成本分摊与结果分析中的“核算负担”“内部成本分摊”“车辆压缩”等生硬或无依据表述；保留排放核算、核算时段等正式用语。PDF25页，检查第3页与第22页；未新增退出机制。
2026-09-12｜中心联盟假设获批：同一配送企业、该企业的配送任务、燃油车与纯电动车混合车队；各中心组成大联盟，分摊成本高于独立配送成本时可退出联盟。第2.1节仅列约束，不展开论述、不加引用；见P177及CURRENT_PROJECT_CONTEXT.md末条。
2026-09-12｜第7页公式引用错位修正：动态信息处理段删除提前出现的式（32）和式（36），车辆改为从虚拟起点按上述状态继续执行剩余任务；PDF25页，公式编号未变。
2026-09-12｜第12页表注删冗：表3注释删除中心与公共充电站不接受配送服务及不设时间窗、服务时间的说明，仅保留车辆作业时段和班次。
2026-09-12｜4.1.1顺序调整：标题不变，正文按算例概况与来源、基础节点表、参数说明与参数表、电网碳强度图、动态事件与动态客户表排列；数据未改，PDF25页。
2026-09-12｜参数含义与两项挂账：电量0指首次中心充电开始时的电量，基准碳配额0指不分配免费配额；电网碳强度说明与图2已同页。公斤需求旧1000 kg折算和逐小时数据／半小时计算网格统一重构见`deferred_ledger_20260818.md`的L12、L13。
2026-09-12｜表5与4.7关系纠正：`dynamic_v7_20260909`使用了表5对应的10条混合事件并形成5个触发批次；现稿不重跑。参照姜广田等2024年算例部分的表前写法，正文说明事件是在上述50个客户的初始算例基础上设置的，列出四类事件后引向表5，不再另述接收时段及抽取过程。证据见`solver/reports/dynamic_v7_20260909/`与`CURRENT_PROJECT_CONTEXT.md`当日更正。

2026-09-13｜引言定稿：不扩篇幅、保留引文顺序与文献内容；加强文献与研究问题的承接，PDF24页。证据入口：docs/paper_v2/paper_main.tex及CURRENT_PROJECT_CONTEXT.md本日记录。

2026-09-13｜引言与实验衔接：问题对应4.3—4.7，不提前写结果；仅改引言6处，篇幅不增、引用不变。证据入口：paper_main.tex引言及CURRENT_PROJECT_CONTEXT.md本日记录。

2026-09-13｜必要文字修订：标点、首次引用顺序、0.20情景依据、初始电量/跨日与行程归属、表11统计、动态状态及联盟成本表述；用户授权本轮Codex直接落稿。PDF24页，全文汉字减少14字，文献内容和公式不变。原有长表分页报错保留，详见CURRENT_PROJECT_CONTEXT.md本日末条。


2026-09-13｜共享约束：2.7为2.5和2.6两模型的共同约束；仅调整标题层级及引导句，PDF24页。


2026-09-13｜四处批注：2.7文献式引导、删表6引导句、充电安排比较对象；碳价1.00既有情景核实，0.20基准不变。证据见CURRENT_PROJECT_CONTEXT.md本日末条。

2026-09-13｜发布包：移动磁盘根目录同论文标题文件夹；独立代码、投稿ZIP、最终结果与许可。公开前发现三处正文/实现口径问题，尚未上传GitHub。入口：CURRENT_PROJECT_CONTEXT.md末条及发布副本USAGE.md。

2026-09-13｜公开完成：用户选择按现状发布、保留差异；GitHub zlxshu/mixed-fleet-logistics，v1.0.0附件已核验。入口：CURRENT_PROJECT_CONTEXT.md公开完成记录。

2026-09-13｜投稿命名与官方规范：标题目录/投稿材料，主稿与源ZIP分开；15页及图表语言口径待编辑部确认，摘要长度已计数。详见CURRENT_PROJECT_CONTEXT.md本日记录。
