...
- [E2--E7 重建结果与待用户裁决（2026-08-01）](e3_e7_rebuild_foundation_20260801.md) —
  当前唯一实验判断正文，状态 `E3_CLOSED__E4_NEW_MAIN_APPROVED__E5B_OPTIONS_READY__E6A_FORMAL_COMPLETE__E7_QIU_FORMAL_OPTIONS_READY`。
  E2 已成功封存并删除正文内部“复现”段；E3 用 Solomon I1 统一合法开局完成六算例×十种子正式批，
  60/60 全部降本，平均 36.477%；E4 三算例×十种子中含碳成本目标平均减排 2.679%、运营成本几乎不变，
  纯碳目标平均减排 13.935%、运营成本增加 2.487%；用户已选新E4作正文主方案；E5-B已定位六类基础问题并形成
  Montoya时间、Wang混合车队联合、修正后默认人民币三项待选路线；E6-A正式60单元/900联盟面板完成，合作节省平均33.018%，60/60核非空，Shapley在核内52/60；
  E7纠正经济拒单后，三触发30/30合法且全部接单，逐单最省成本但改路线最频繁，固定30分钟少调整且排放最低，
  邱式位于中间并显著缩短固定方案里程；邱与姜均无可换算的评价预算，现有8次排障值不升级，正式求解路线和预知需求参照的选项已列明待用户拍板。

- [E5-B 文献与基础设施诊断（2026-08-01）](../e5b_literature_and_infrastructure_diagnosis_20260801.md) —
  当前 E5 零效应没有证伪研究问题。代码与文献共同确认目标稀释、曲线不进路线、充电量非决策、站点访问自建限制、
  满电离场漏记电费和独立弱搜索污染；逐页对照 Montoya 2017、Wang 2025/2026，给出三个不改 China81 客户和站点的
  实质敏感性选项。任何会改变正式参数、决策空间或故事的选择仍由用户审批。
...
- [未决问题清单（2026-07-31）](../open_issues_20260731.md) —
  状态 `E3_CLOSED__E4_NEW_MAIN_APPROVED__E5B_OPTIONS_READY__E6A_FORMAL_COMPLETE__E7_QIU_FORMAL_OPTIONS_READY`。E3、E4主线已关闭；E6正式面板完成并只剩Shapley或核仁结算终点选择；
  E5只保留三种敏感性具体实现选择，E7已关闭拒单/子集枚举并只剩正式求解路线与预知需求参照选择；
