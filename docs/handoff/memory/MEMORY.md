...
- [E2--E7 重建小试结果与待用户裁决（2026-08-01）](e3_e7_rebuild_foundation_20260801.md) —
  当前唯一实验判断正文，状态 `PILOTS_COMPLETE__E3_E4_E6_CANDIDATES__E5_FAILED__E7_METHOD_DECISION`。
  E2 已成功封存并删除正文内部“复现”段；E3 用同预算比较历史客户归属固定与算法重分客户，小试平均
  降本 37.823%；E4 联合优化在运营成本不变时系统减排 2.819%，隔离重跑复现；E5 虽 120/120 合法，
  但 FS 与 PL 在实际充电区间物理相同，当前微小差异不能归因于非线性，现版本判失败；E6 15/15 联盟
  合法且大联盟降本 35.379%，但自然账本无人比单干差，实体调货模型尚未闭合；E7 50 客户十种子
  40/40 合法，母体式混合触发平均优于固定 30 分钟 1.73%，但文献没有现成的可扩展批内永久拒单算法。
...
- [未决问题清单（2026-07-31）](../open_issues_20260731.md) —
  状态 `PILOTS_COMPLETE__E3_E4_E6_CANDIDATES__E5_FAILED__E7_METHOD_DECISION`。A 类现只保留 E3 历史客户生成、E4 新旧主次、
  E5 改时间目标或退出、E6 直接合作或实体调货、E7 三种可扩展接单路线五组真实选择；
...
- E5 Montoya FS/L1/L2/PL 两算例×三种子×五容量共 120/120 合法，但 FS 实际充电全部位于与 PL
  相同的第一段，约 0.1% 差异是搜索波动，当前版本判失败；待用户选择改用 Montoya 时间目标重跑或退出。
- E6 15/15 联盟合法，大联盟降本 35.379%，Shapley 在核内；自然账本四家均优于单干，方法 B 无新增
  效应。目标期刊直接合作故事可用；实体调货仍缺货物承运、多趟拆分、车辆共享和费用归属语义。
- E7 50 客户十种子 40/40 合法；混合触发相对固定 30 分钟均值低 1.73%，逐单触发均值低 7.16%。
  当前接纳子集穷举扩到 100/150 客户会达到约 104 万/10 亿组合；逐页核查未找到“批内永久拒单且
  不穷举”的现成算法。混合触发主角已定；待用户在逐单到达顺序、批内顺序搜索小试、只保留 50c 中选择。
