...
**🧪 2026-06-21 [x86/DR] reduced-budget pilot07 = WEAK (DR lane 最新)**: 接 [[project-plan-overview]] 迁移后，x86(`dr-x86`)按"小/快设置训、降预算评"路线跑趋势 pilot(非全预算)。配置 `eval_budget=1500/block_size=64/num_actors=6`(校准拒 1500/32/8=内存13.76GB、拒 3000/64/6=<100ep)，只训 100_02、held_out=100_03、formal=100-01；136 ep、route→energy→carbon、零违约、device=cuda、shared baseline 开。评测同降预算同 py313 worker：ppo_block vs alpha_ucb_block 相对% = 100-01 +1.63% / 100_02 -4.77% / 100_03 -6.06%，未稳过 random_block、checkpoint trend 不支撑 → **WEAK**。**印证此前判断**(见上文 90/92 行)：DR 在算子选择上对 AlphaUCB 无稳定 headroom(松/降预算下增益薄或为负)。报告/模型 x86 本地 `solver/reports/dr_alns_ppo_v3_block_dr_alns/async_pilot/pilot07/`(最小集已 git add -f 回 dr-x86，tip `101a4819`)。论文主线=winner kernel + 机制；DR 归 future-work、不卡论文。

**🔬 2026-06-22 [x86/DR] pilot08 WEAK + pilot09 headroom 诊断（DR lane 最新，承上 pilot07）**: pilot08=课程小→大（8422ep/12 小算例 25+50c/无塌缩/entropy7.11→5.90/选 checkpoint0240）后 900s 等墙钟 vs AlphaUCB = +0.69%(train)/+0.24%(held) ≈平手 → **WEAK，坐实"PPO 天花板≈AlphaUCB"**（90/92/141 行旧判断证实：算子选择对好 bandit 无稳定 headroom；且这次训练全做对仍持平=更有结论性的负结果）。pilot09（文献优先：Zotero PASS + Daysalilar/Reijnen DR-ALNS/Narayanan/Wan，证据 log 带 key/URL）三招诊断：stochastic PPO held **-3.65%** 更差、deterministic 仍≈平手、**AlphaUCB-meta 屏（固定算子=AlphaUCB、只扫 q/thr/exp）最优静态比默认 AlphaUCB +3-4.6%**。**重定位：DR 的 headroom 在搜索参数（q/threshold/exploration）、不在 destroy/repair 选择** → 救法=小动作空间 block_meta（固定算子、只学搜索参数）。**Claude caveat（关键）**：meta 优势=更优静态参数（默认 AlphaUCB 欠调，拿它当对手是 strawman），block_meta 必须**打过"最优静态 meta"**才算学习贡献（否则只是重调参=minor），且需 900s 复核（屏在 budget600+best-of-45 选择偏差）。gate=REFINE_ALPHA_UCB_META_NEXT，未触发 HALT_DR_HEADROOM。工具 `pilot09_rescue_tools.py`（untracked）。

**✅ 2026-06-27 [x86/DR] Pilot16/Pilot17 全规模救援结论（DR lane 最新）**: 按文献式“小动作空间搜索控制”重搭后，Pilot16 全规模三班倒长训本身健康：`50/75/100/150/200c × 3`、11280 episodes、route→energy→carbon、零违约、py313/NumPy2.3.5、CUDA、entropy 不塌、candidate 非默认≈64.5%、search-control 非 continue≈49.6%。但 Pilot17 同预算正式评测只得到 **ACCEPTABLE_PASS，不是 TARGET_PASS**：对最强外部基线（default/tuned AlphaUCB、winner/random/SA/official 中按规模取最强）相对%为 50c +0.12、75c +0.38、100c +1.01、150c +0.99、200c +0.36，全规模平均 +0.57%，远低于 user 目标 +10%。更关键的症状是 PPO 120 行都提前 stop（约 8-9.3/20 eval），best=final/update375 与后期 checkpoint 几乎同值，说明 search-control 在低预算里学成了“早停/浅层收益”而不是真正把后续 ALNS 搜索做强。**根因更新**：旧问题“算子选择无 headroom”仍成立；新结构解决了可训练性和全规模覆盖，但引入 stop/reward credit assignment 捷径，学习贡献仍太小。若继续救 DR，下一轮必须先做 stop/acceptance reward 医治和独立 held-out 全规模三班倒算例，再谈长训；否则 DR 只能作为负/弱正实验，论文主线回普通 ALNS + 强基线。
