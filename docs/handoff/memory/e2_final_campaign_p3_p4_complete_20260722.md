...
- 模型章V7候选已把动态触发、客户更新、实体车状态和历史成本/排放/收益累计前移到
  问题定义；正式结构固定为问题与动态、符号、能耗、充电、成本与总目标、约束条件，
  总目标在所有约束之前。公式标签和科学语义不改，构造情景边界补明，独立编译无警告。
- 表4文字另立零搜索自动入口：S4/S5精确PASS后只从封存路线表生成路线数、客户数、
  成本、能耗、排放和装载率说明，禁止手填或替换解。整稿接入器锁定主TeX和候选源
  指纹，只生成独立候选稿；发布链、E2结果文字门和表4文字门任一未通过即停止。当前
  无结果结构检查通过，未改主TeX、算法、正式结果或运行中的合同。
- 另冻等待式论文候选链：既有发布链精确PASS后才运行两类文字物化、独立整稿合并和
  出版社格式编译检查；不覆盖主TeX、不启动E3。编译PASS只形成待逐页目检候选，不能
  越过主稿合并和E3放行门。
