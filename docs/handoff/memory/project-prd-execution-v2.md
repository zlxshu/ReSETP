...
2026-07-30 E7 v2 启动层状态：上一轮 `instance_json` KeyError 已以 China81 八类
真实源路径/哈希的 fail-closed 适配器修复，零搜索检查通过。但共享初始计划仍漏传
NL90 的 China81 prices；诊断性显式传参后又确认碳感知前夜充电缺
`charge_day_offset=-1` 权威曲线。按崩溃立即停规则，本轮搜索0、预算未锁、三规模
正式未跑，四臂均为共享启动层阻断。不得把三机制 false 解释为无参与，也不得用当日
曲线复制前夜。新尝试须经用户授权，同时补齐显式 prices 和权威前夜日历，并先过
零搜索契约门。证据=`baselines/china_e3_e7/e7_dynamic_v2_20260731/`。

2026-07-20 MPILS-MVNS第二候选两发协议终局：首发0/0/3后，A组三次母体按预登记
15%公式把W/冷却从128唯一修订到1666；没有候选参数网格。第二发触发从28--39次
压到2/0/2、迭代恢复到母体约92.5%/103.6%/91.1%，但性能仍1胜0平2负：
PR11B -0.430%、PR17B +0.000612%、PR21B -0.180%。PR17B微胜时专用动作零触发，
三题专用动作刷新最好均为0。按不可救援合同判
`STOP_MPILS_MVNS_C2_REPLACEMENT_LINE_AFTER_SECOND_FIRE`；G2、28题、扩种子、
China81、阶段二和论文主张均未放行。权威说明和memory节点已落盘。

2026-07-20 MPILS-MVNS第二候选G1 B组首发：用户批准
PR11B/PR17B/PR21B、seed1、8.5秒同批母体，零负且至少一胜。冻结提交链为
G0/源码`2f434816`、G1预登记`3fb6c925`、搜索前运行器修复`d013d892`；两次启动
失败都在导入或Git祖先检查处停止，未创建结果或进入求解。正式首发六解独立有效，
墙钟/五机制/限额/保护文件全过，但性能0胜0平3负（-0.666%/-2.404%/-3.259%）。
候选只完成母体39.3%--50.7%迭代，28--39次昂贵段搜索是明确活动开销病灶；
PR21B 28次均无候选仍丢61%迭代。按预登记允许A组一次W/冷却常数诊断和一次修订，
不得扫参数或加模块；B组第二发再败即终止。证据与判读见memory节点。

2026-07-20 MPILS-MVNS第二候选G0基础设施收口：用户允许尝试Claude安排，除明确错误
或累赘外保留。实际保留PyVRP 0.13.4 ILS母体、事件式三视角记忆、替换式多车场连续
段扰动、母体局部精修和五机制常驻；纠正PR11A单种子因果夸大、AILS-II固定1/2/3段
误引、PyVRP现成回调误判以及China81算子直接迁入公开题四项。按MIT迁入并修改上游
Python ILS小循环，C++局搜和计分器不改。PR11A固定5/1000/5000迭代逐位等价且独立
验解通过；5000迭代×5的上游/迁入/常驻中位墙钟4.137077/4.157232/4.170648秒，
常驻开销0.811%；人工门证实一次替换时原生扰动0调用、零扰动精修1调用，连续段移场
完整可行且64/每段6/48/12上限全守。首次G0只因MIT文本多一个末尾空行FAIL，原包保留；
零求解补正后许可证逐字节一致，有效判
`PASS_MPILS_MVNS_C2_G0_AFTER_LICENSE_REPAIR`。性能状态仍UNKNOWN；没有运行B题、
确认题、28题、China81或阶段二。终审再删除未使用导入和多余末尾空行；首次源码卫生
补正因少算一个字节差异且缓存旁文件未清而FAIL并保留，v2以旧哈希精确确认两处均无
执行语义，重过Ruff、内存编译、5迭代等价和行为门，当前源码判
`PASS_MPILS_MVNS_C2_G0_CURRENT_SOURCE`。权威记忆=`docs/handoff/memory/mpils_mvns_c2_g0_20260720.md`。

2026-07-19 算法比较基础 v3 拨正：用户要求先像陈雨蝶一样选有资格的经典强手、
当前开源强手和最新相近混合体，再由它们共同使用、带BKS的原始开放题反推公开赛场，
不能先定CVRP或X题再拼对手。现行公开主场为`V13-MDVRPTW-28`，公开表按职责比较
当前可验BKS、VCGP/HGSADC、PyVRP 0.13.4、MDFIHA、MDFIHA-ETGA和本文完整算法；
`PR17A`单独解释母体、独立破坏重建/精修、部分混合和完整混合的血缘。开发6题、
确认6题、其余16题封存，划分不看算法结果。China81是唯一完整模型赛场，后续只比
同一ReSETP完成器下的母体、独立机制ALNS和完整混合体。零搜索包28/28原题语义、
BKS双检查和论文值抽取通过，当前可验BKS在28/28上优于2026论文值；同路径三臂
逐位一致、组件调用0。当前判
`PASS_FOUNDATION_AND_SAME_PATH__PERFORMANCE_SEARCH_NOT_AUTHORIZED`，未跑性能、
China81或阶段二。X-100审计只作未入选候选历史证据。详见
`docs/handoff/algorithm_comparison_foundation_v3_20260719.md`、
`docs/handoff/mechanism_hybrid_algorithm_blueprint_v2_20260719.md`和
`docs/handoff/memory/algorithm_comparison_foundation_v3_20260719.md`。

2026-07-19 结果优先混合算法阶段一执行终局：用户把范围收紧为“公开只用带BKS的
原始开源题、私有只用China81”。`SEG-GEN-02`有界机制段生成的单测、接入冒烟和
零搜索行为门通过；公开三组互斥Solomon+BKS开发门中，两个十秒融合候选都明显胜
项目ALNS但未总胜纯PyVRP HGS。第一版对HGS为3胜2平1负却因R111多一辆车汇总失败；
双精英版对HGS为2胜1平3负。两者停止，不造第三候选、不跑56题。China81运行器因
G1/最终冻结未开且公开无强阳性，在导入求解器前HALT，搜索0。AppleDouble无重跑
修复确认普通证据文件旧哈希不变，权威独立审计44/44通过；机器状态仍是
“基础设施通过、算法硬目标未达成、阶段二未启动”。详见
`docs/handoff/memory/outcome_first_hybrid_20260719.md`。
...
## 2026-07-19 机制成段生成最小性能门

外部报告把既有失败统一诊断为“机制从未进入生成”。源码和历史证据复核否定该绝对说法：
仓库已有跨场/碳生成算子、机制初始化与连续专家。剩余可检验缺口缩为v7单客户责任邻域
覆盖不到的2--3客户连续段联合跨场搬移，且不能先按里程否决。

`SEG-GEN-01`行为门通过后，在结果盲donor03两个新题、三臂、共同seed1、每臂B100执行
一次性预警门。实际54客户题三臂同为`1009.159041`；实际108客户题机制臂
`2929.437620`，优于控制/里程臂`2966.646148`，改善`1.254%`，并真实采纳一段3客户、
路程增加但完整成本下降的调整。质量条件全部通过。

当前实现仍按预注册淘汰：机制墙钟中位为控制的`3.298`倍、最大`4.694`倍，超过
`1.25/1.50`上限；108客户题一次扫描1270段、10420插入位和10483次可行性检查。
判`STOP_SEGMENT_GENERATOR_NO_RESCUE`，冻结本穷举候选，不在已见题上调参，不扩三种子、
正式benchmark、China81、E2--E7或阶段二。结果只支持“生成侧有质量信号但当前工程实现
不可用”，不支持“生成侧整体失败”或“算法优化完成”。

## 2026-07-19 HGS-AEALNS-03 公开最小门止损

用户批准一次新内嵌候选后，`EA-HYBRID-004`仅在六个新的原始Solomon+BKS题上比较
纯PyVRP HGS与精英分歧自适应ALNS，单线程、共同seed1和初解、每臂5秒。候选为
2胜2平2负；R109/R208缩短距离，但RC105为23路而纯HGS为15路，RC205亦略负。
机器判`STOP_HGS_AEALNS_03_ANY_LOSS`，公开必须零负HGS的目标未完成。

独立零搜索审计60/60通过；时间、可行性、BKS后关联、输入与保护面均无事故。源码和
两个零搜索构造测试确认当前修复排序缺时间窗与新车固定成本语义，普通破坏允许增路，
解释了紧时窗碎路线风险。当前候选冻结；正式56题、China81、E2--E7和阶段二均未放行。
若继续须用户另批`EA-HYBRID-005`，修安全语义后只用一组全新留出题，不能在已见六题
调参救援。

## 2026-07-19 ReMIX V13 六题开发门收口

公开比较基础已按陈雨蝶式逻辑改为 `V13-MDVRPTW-28`，China81 保持唯一完整模型
赛场。同路径关闭门、PR17A 行为门和 HGS 路线供应桥均通过，随后执行六题单种子、
单线程、每臂十秒开发门。

完整 ReMIX 对六题较强可运行核心为 `0 胜、0 平、6 负`，新 BKS 为 0；六题较强
核心全部是 PyVRP 0.13.4 ILS。全部 18 解独立有效，全部墙钟合规，保护源码未变，
AppleDouble 清理后 34 件登记证据哈希复算失败 0。原脚本 `HOLD_MIXED` 标签不符合
“有胜有负”的合同语义，权威判定为
`STOP_CURRENT_REMIX_V13_CANDIDATE_ALL_LOSS`。

统一死因是削短 ILS 预算后，HGS 首解、五轮路线交换—拆解重建—重复局部搜索和末尾
路线会审没有补回损失。PR11A 路线会审对缩短版 ILS 仅改善约 0.074%，仍输完整预算
ILS；其他五题没有净改善。当前停下复盘，不开确认块、完整 28 题、China81 或阶段二。
若另批继续，只允许先验证 ILS 主循环内的 ALNS 式多车场/时间窗扰动替换，不再堆并排
工种。权威尸检见
`docs/handoff/remix_v13_development_forensics_20260719.md`。

## 2026-07-20 PyVRP V13 原装母体时间—质量诊断

用户批准先查母体潜力，不改算法。预冻结后仅运行PR11B/PR17B/PR21B、seed1、
PyVRP 0.13.4 ILS默认参数、8.5/60/300秒。九解独立有效；三题gap依次为
`2.115%→0.469%→0.215%`、`3.022%→1.070%→0.720%`、
`3.510%→1.594%→1.007%`，两个时间段均严格改善，判
`TIME_CONTINUES_HELPING_ALL_THREE`。

该诊断证明此前8.5秒不足以判断母体上限，当前不应仅据短门换掉PyVRP。它不撤销C2
替换式扰动STOP，也不是正式28题、新BKS或调参收益证据。下一算法任务若获批，应以
PyVRP强母体的受控配置/长预算研究为入口；完整28题、China81和阶段二仍未放行。

## 2026-07-20 China81 非线性充电核心 NL0

公开参数竞速和多阶段双节奏均按冻结门停止后，执行转向 China81 的唯一物理阻断。
分批合同先新增唯一纯数学曲线内核，不改成本、检查、价格、评价器或主 TeX。

生产内核对既有冻结 264,600 行动作全量复算；261,027 个可行动作的持续时间和分时
电量与冻结模块均零差异，L100 恒功率退化网格失败 0，24 项目标测试、Ruff 和保护
文件零差异通过。判 `PASS_NL0_PRODUCTION_CHARGING_CURVE_CORE`。该 PASS 只放行
NL1 的解结构、多趟排班和证书接线；China81 正式搜索和性能结论仍为 false。

## 2026-07-20 China81 非线性充电排班 NL1

解动作已向后兼容地增加起止电量和曲线身份，多趟证书冻结参数哈希；排班中的趟间可达
电量、后向最小补能、持续时间和首趟预充均改为调用唯一曲线内核。两趟固定夹具在
L100 下需要 1 辆实体 EV、在 NL90 下需要 2 辆；新动作持续时间与曲线最大误差约
`3.98e-13` 秒、电量误差 0，恒功率控制最大漂移约 `3.13e-13` 秒。84 项相关测试、
Ruff、持久化兼容与保护边界通过，判
`PASS_NL1_CURVE_AWARE_MULTITRIP_SCHEDULE`。全量回归 12 个基线/旧契约失败未被
掩盖；成本、碳、检查和 China81 道路 profile 仍未闭合，正式搜索继续 false。

## 2026-07-20 China81 非线性充电成本—检查 NL2

充电修复、半小时槽电量、碳择时、成本、排放、利润和独立检查器已经统一读取动作起止
电量与曲线身份。高 SOC 动作的真实首槽电量为 `8.333333333333334 kWh`，旧均匀
摊分为 `5.555555555555555 kWh`；缩短一分钟和缺少元数据的非线性动作都在成本端与
检查端失败关闭。L100 新旧动作逐位一致。零搜索碳择时夹具从
`0.7265247116695909 kg` 降到 `0.12108745194493127 kg`，仅证明机制接线。

目标回归和 Ruff 通过；全量回归仍为 12 个冻结提交可复现失败，未称全绿。权威判定
`PASS_NL2_NONLINEAR_COST_CHECK_AND_TIMING`，只放行 NL3 的动态继承与 China81
三道路 profile 适配；正式搜索、三题性能门和 81 题仍为 false。

## 2026-07-20 China81 非线性充电动态继承 NL3a

- 动态续排中的可达电量、最早发车、趟链后向电池账、动作持续时间、证书验证、低碳
  择时和滚动切割已统一调用非线性曲线；新动作和证书冻结起止电量、曲线 ID 与参数
  哈希。
- NL90 夹具 2 个动作的独立时长误差为 0，缩短一分钟和缺元数据均失败关闭；动态
  低碳择时从 `1.028968610541679 kg` 降到 `0.12105513065196208 kg`，运行中切割只
  在动作结束释放且不重复计电。
- 22 项相关回归和 Ruff 通过，旧恒功率动态公式计数归零，L100 误差为 0。权威包
  `baselines/model_verification/china81_nonlinear_dynamic_nl3a_20260720/` 判
  `PASS_NL3A_NONLINEAR_DYNAMIC_INHERITANCE`。
- 本门零搜索且不代表算法胜出。下一步只接 China81 的 CV/EV 两类车辆 profile，
  每类严格使用距离、时长和 Σ(v²d) 三个有向矩阵；三矩阵不是三种车辆。

## 2026-07-20 MV-HGS-SP 阶段一算法结构冻结

算法身份纠正并冻结：PyVRP 0.12.2 才是真 HGS，0.13.4 是 ILS。当前候选
`MV-HGS-SP` 让三个真正 HGS 种群从燃油、简化电动和完整机制三个视角生成路线，
由完整非线性 ReSETP 模型重排精英，再用 SciPy/HiGHS 集合划分精确重组路线；重组
不严格改善即保留最好的完整父方案。

H4 开发门 2 胜 1 平 0 负，H5 新题门 2 胜 1 平 0 负，H6 三题三种子
5 胜 4 平 0 负；75 客户层 3/3 改善，150 客户层 2/3 改善 1 平，
25 客户层全平。九解全部可行，34 项相关测试和 Ruff 通过，H6 清单哈希全匹配。

该 PASS 只冻结私有方向结构。候选比单个母体使用更多 HGS 与精确重组算力，尚无
等墙钟纯 HGS/ALNS、China81 全量或 V13/BKS 证据；公平/动态和 ALNS 贡献未证。
H0 早期适配器哈希存在后续扩展漂移，H6 当前源码哈希闭合。裁决
`PASS_PRIVATE_DIRECTIONAL_ARCHITECTURE_FREEZE`，不放行正式搜索、阶段二或
论文优胜结论。

## E2-RERUN-UNIFIED-01 第 0 步第三、四条（2026-07-27）

- 结果盲收敛试跑已完成 45/45 单元（9 题 x 5 臂），seed 1，6 个独立
  `spawn` 进程。MV 三视角各自完整使用 `NoImprovement(3000)`。
- K=3000 的 95% `L/S < 0.5` 门失败（20/45）。建议下一个候选用
  `K(n)=max(5000,110n)`，已观测轨迹上为 43/45；不缩放的对应全局值为
  K=21000。两者都未作额外重跑验证。
- 大规模层 15/15 为晚改进；小规模层中位 S=3101，记为早停/饥饿指纹。
  本阶段不含臂间成本或性能结论。权威目录为
  `baselines/e2_rerun_unified_01_step0_20260727/`；第 1 步 2025 单元未授权。

## E2-RERUN-UNIFIED-01 正式批洁净重启（2026-07-27）

- 首次正式批因两个 6-worker 池重叠而作废。限时 MIP 的同墙钟求解工作受到
  不对称压缩；安全停止时 165 个完整尝试行已整体归档，不进入任何正式统计。
- 洁净批从零以 4 workers 启动。五臂 2025 单元、K=3000 起步、按臂×规模层
  L/S 饥饿加倍最多两次、MV 三视角逐轮独立收敛、外层最多三轮等合同不变。
  外部实验池出现时必须暂停；正式哈希清单排除 `.monitor` 与受污染归档。
- PID/PGID 23367 已健康运行，首个原子检查点为 3/2025；监控 findings=0，
  新行包含 S、L、L/S、CPU、墙钟、峰值 RSS 与柴油价绑定。不得提前作胜负结论。

## PAPER-ALGCHAPTER-01 算法章写作（2026-07-27）

- 正文与主流程图已接入正式外层轮次协议：三个视角每轮分别收敛、路线池跨轮累积、
  限时 MIP 路线池集合划分、`min()` 安全网、最好解写回及外层连续无改进停机。
- 第4.2节只改算法描述和证据边界，不改任何实验数字或表占位符；旧封存表图不等于
  新协议验证。报告为 `docs/handoff/paper_algchapter_01_20260727/report.md`。
- 当前编译状态为 `WRITING_COMPLETE__HALT_XELATEX_RUNTIME_UNAVAILABLE`，尚未取得
  新 PDF 页数、错误数、未定义引用数或 overfull 数；恢复 XeLaTeX 后必须补齐两遍
  编译和流程图页面检查，不能引用 2026-07-24 的旧 PDF 作为本任务证据。

## 2026-07-29 E3/E6 正式实验前置门 E3E6-GATES-01

- D2-A 在当前 K7 1735/2565 kg 与 ES1 1700/2600 kg、77.28 kWh 绑定下完成
  81 实例、144 个逐场 `R_d` 重算和全燃油零搜索 witness 重放，判 PASS；旧权威
  只读对比且没有覆盖。
- D3 当前源码四层守卫重新闭合，但严格逐实例条件只通过 45/81：余下 36 个实例
  仅有一个车场，处理臂无法产生跨场服务。因此根判决
  `HALT_AT_GATE_2_D3`，旧结构性 PASS 不得替代本次强门。
- D4 依上游停止规则未运行；没有臂间成本读取或结果报告。当前 E3/E6 源码路径已
  静态确认为固定 80 次完整候选评价而非 `NoImprovement`。正式 E3/E6 继续冻结，
  等待用户明确裁定单车场实例适用域或处理臂定义。

## 2026-07-29 E3-RESPONSIBILITY-MISMATCH-01 更正后执行状态

- 用户已将 D3 适用域更正为多车场实例；当前源码门二在 45/45 个多车场实例上
  PASS，36 个单车场实例按适用域排除，旧 PASS 未复用。
- 主展品 `cn-prd-50c-01-V2-LOCATIONS` 的 0/25/50% 三档输入预检均在 D2-A
  上限内 0 违约，但稳定性面板预注册未封口：当前 50% 哈希错配使
  `cn-jjj-100c-01-V2-LOCATIONS` 天津场需 10 条路线而上限为 7；12 个三/四
  车场实例还缺经批准的目的地规则。
- 判决为 `HALT_PREREGISTRATION_METHOD_APPROVAL_REQUIRED`。防饥饿 pilot 与
  正式搜索未启动，臂间成本差异未查看。后续不得自行改变 D2、稳定性子集、部分
  服务语义或映射条件，须先取得用户批准。

## 2026-07-29 E5-NONLINEAR-CHARGING-01 盲 pilot HALT

- 批准的 50c-01（种子 1--10）、100c-02（种子 1--5）、L100/NL90 两臂和
  共同非线性复算合同均未改变；150c-01 继续排除。
- 32/56/80/160/240 五档结果盲 pilot 全部饥饿，两臂各档 `L/S>0.5`
  单元数为 12/11/12/13/13（各 15 单元）。未读取目标值或臂间成本差。
- 160 与 240 档中，50c 的同 9 个种子对两臂均在 `S-1` 最后严格改进；当前
  扩档只增加最终路线池重组前的候选，结构上不能让该改进获得后续评价余量。
  判决 `HALT_PILOT_STARVATION_SCHEDULE_NOT_CONSTRUCTIBLE`；320 和正式搜索为 0。
- 继续 E5 必须先批准重组后确定性完整候选评价调度，或批准另一饥饿定义。四个
  科学端点全部 `NOT_RUN_UPSTREAM_PILOT_STARVATION_GATE_HALT`，论文不得回填方向。

## 2026-07-30 E5 v2 收敛探针技术 HALT

- 新权威合同 `experiment_contract_v2_journal_aligned_20260730.md` 已废止上述
  L/S、20% 和 pilot 门禁并授权 E5；新 runner 不再包含这些准入逻辑。
- 1500 次长探针在 50c-01 seed1 L100_control 上实际完整评价 331 次，
  expected=1500，触发 `HALT_E5_V2_NONEXACT_BUDGET`。配置 archive 大小是上限，
  不能保证真实唯一候选数量，故当前上限映射不是精确完整候选预算。
- 按用户技术故障停止规则未修复或重试；50c/100c 正式单元 0，四科学端点均
  `NOT_RUN_UPSTREAM_TECHNICAL_FAILURE`。权威目录为
  `baselines/china_e3_e7/e5_nonlinear_v2_20260730/`；继续须用户另批精确预算
  消费的技术处理方案。

## 2026-07-30 E5 v3 上限预算修正与探针歧义 HALT

- 用户已纠正 v2 的自建预算语义：完整候选预算是 cap，不是 quota；
  `consumed <= cap` 正常，`consumed > cap` 才 HALT。v3 runner/checker 已按该
  语义建立，保留 v2 的实验结构和日志，未改搜索语义或保护文件。
- 1500-cap 探针首个 50c-01/seed1/L100 单元在搜索返回后因 trace 中 null
  objective 触发 `HALT_E5_V3_INVALID_COMPLETE_TRACE`。异常发生在单元 trace
  原子落盘前，本轮实际消费次数和改善曲线均未保存。
- `epochal_hgs.py` 的既有 schema 会把捕获到的候选不可行或错误合并记为
  `INFEASIBLE_OR_ERROR` 且 objective 为空。现场无法区分合法不可行候选与
  翻译/数据技术故障；按用户“不确定则停止”规则未修复或重试。
- 根判决 `HALT_PROBE_AMBIGUOUS_NULL_OBJECTIVE_TRACE`。正式共同预算未选，
  50c/100c 正式搜索 0，四科学端点回答数 0。权威目录：
  `baselines/china_e3_e7/e5_nonlinear_v3_20260730/`。

## 2026-07-30 E5 v4 执行状态

- 用户授权的诊断顺序与正式实验已执行：trace/status 先原子落盘，再校验；
  `epochal_hgs.py` 只增异常类型/消息，并以同 seed 逐位指纹证明搜索语义不变。
- 双臂 1500-cap 探针把 368/368 个 null 行识别为合法完整模型不可行，
  technical error=0；共同正式 cap=400。50c 完成后才启动 100c，10 seeds×2 臂
  共 40/40 正式搜索 PASS，独立 checker 40/40 PASS。
- 终端证书 loader 因未排除 AppleDouble，把 40 个真实 JSON 与 40 个 `._*`
  读成 80 并崩溃。按用户“崩溃即停”规则，端点聚合未继续，四个端点均
  `NOT_AGGREGATED`。状态
  `HALT_APPLEDOUBLE_CERTIFICATE_DENOMINATOR`；后续修证书枚举/重聚合须新授权，
  不得覆盖 v4 终态。

## 2026-07-30 E5 v4 已认证单元终局聚合

- 用户已明确批准一个独立 final 轮次：修正产物枚举、清理 v4 AppleDouble，并仅从
  40 个既有 plan/certificate 聚合；正式搜索与独立 checker 均未重跑，
  `search_reruns=0`，v4 HALT 目录和真实文件未覆盖。
- 所有相关枚举统一排除 `._*`、`__pycache__`、`.pytest_cache`。v4 的真实文件
  清理前后均为 262，40 plan/40 certificate 分母不变，68 个旁文件清除后为 0，
  逐文件 SHA-256 集不变。
- 数据交叉检查 40/40 通过且矛盾 0。NL90 臂可行率总体 20/20；L100 假可行
  0/20；共同 NL90 可行的 20 对成本变化均为 0。196 个充电会话仍显示物理时长
  暴露：NL90−L100 均值 232.270 s、中位 0、最大 1264.582 s。
- 判决 `COMPLETE / MECHANISM_BUT_TIE`。本场景下线性近似没有高估可行性且成本
  效应为零；不得外推到更高 SOC 或更紧时窗场景。权威目录为
  `baselines/china_e3_e7/e5_nonlinear_final_20260730/`，四件套、报告和最后
  `done.json` 均已闭合。

## 2026-07-30 E5 文献曲线预注册暴露门停止

- 仓库内核可接受任意分段参数；`NL90_mild` 和 `NL80_stress` 是合成配置，不是
  特定车辆实测曲线。Montoya et al. (2017) 与 Froger et al. (2019) 第 4 页图 1
  锁定 22 kW normal 曲线节点 `(0,0)`, `(0.62 h,13.6 kWh)`,
  `(0.77 h,15.2 kWh)`, `(1.01 h,16 kWh)`；VRP-REP 120 个 XML 机器复核一致。
- 预注册 `M17_22KW_NORMAL_PWL` 的 SOC 断点为 0/0.85/0.95/1，三段功率为
  21.9355/10.6667/3.3333 kW。参数基于物理与文献，在任何新臂目标值之前一次
  锁定；不采用合成 `NL80_stress` 冒充文献曲线。
- 只读复算最终 E5 的 196 个会话：85% 首折点仍只影响 36/196（18.367%），
  50c 为 20/80，100c 为 16/116；80%、85%、90%、95% 的计数完全相同。按用户
  预设的低暴露停止门，判 `SKIPPED_CURVE_ALSO_UNEXPOSED`。
- 搜索单元 0/20，未观察新臂成本、可行性、车辆数或时长差；没有三臂结果表和
  新臂四端点，不得把未运行写成零效应。既有 final E5 目录未修改、未重跑，其
  COMPLETE / `MECHANISM_BUT_TIE` 结论继续有效。
- 权威目录：
  `baselines/china_e3_e7/e5_literature_curve_20260731/`；详细记忆：
  `docs/handoff/memory/e5_literature_curve_20260731.md`。

## 2026-07-30 E3 分区配送对联合配送正式收口

- 用户裁决以 ZONE/JOINT 两臂替代恒等的 IND/ZONE/JOINT：两题行政
  `customer_home_depot` 与有向道路最近车场完全一致（50c 0/50、100c 0/100），
  因此 IND 没有独立输入差异。四份输入、共同初解与 cap=400 锁定后，先完成 50c，
  再完成 100c；两题×两臂×10 seeds 共 40/40 PASS。
- 旧硬锁真错误定位到 `epochal_hgs.py::_run_exact_epoch` 终端种群档案：14/14
  跨场完成候选被误抛异常。修复只把这类候选在完整补全后正常筛除并计入实际消费；
  JOINT 50c seed1 改前/改后最终解哈希、完整目标哈希和浮点位逐位一致，四个保护
  文件未改。
- JOINT 相对 ZONE 降本为 50c 2.272759%、100c 0.693115%，两算例等权总体
  1.482937%；车辆均值 9→8、18→17.2，但距离增加 28.035662%/12.176611%，
  碳排增加 4.044894%/1.182531%，硬时间窗满足客户数均不变。效应低于期刊同类
  3%--6% 参照，结论是地理最优分区后的剩余降本协同有限且伴随排放权衡。
- 独立 checker 逐解复算 40/40 通过，清单 167/167 通过；权威目录为
  `baselines/china_e3_e7/e3_zone_joint_20260731/`，详细记忆为
  `docs/handoff/memory/e3_zone_joint_20260731.md`，最后 `done.json=COMPLETE`。

## 2026-07-30 E6 成员收益核算闭合、F 候选池缺失停机

- I/U 不开新搜索，分别复用 E3 同 seed 的 ZONE/JOINT 最终解；成员账本沿用
  `solver/src/setp_solver/profit.py`，没有新造分摊规则。20 个 seed 单元的
  成员成本分摊与完整系统目标全部闭合，θ=1 下 U 自然满足所有成员不吃亏为
  0/20（50c 0/10、100c 0/10）。
- 现存 E3 trace 未保存嵌套候选解或成员账本，只有逐候选总成本/状态；每个 U
  单元仅保存最终一个方案。因此 F 的最低成本公平解、公平代价和弱势成员改善均
  不可计算，判 `HALT_E3_NESTED_CANDIDATE_SOLUTIONS_NOT_PERSISTED`。
- 本轮 `search_reruns=0`。继续须用户批准对 20 个 U 单元做候选持久化回放，
  保持原搜索语义并以最终 U 解哈希等于封存 E3 为硬门；不得直接把 I 回退写成
  所有 binding 单元的最优 F。

## 2026-07-30 E6 候选池持久化回放与 F 完成

- 用户已授权只回放 E3 JOINT 的 20 个单元并保存完整候选本体；ZONE 搜索复用 E3，
  重跑数为 0。cap=400、最多 2 workers。20/20 回放的最终解哈希、完整目标浮点位、
  实际评价数、分类 trace 和终止原因全部与 E3 封存值一致，搜索语义与受保护文件
  哈希未改变。
- 候选证据为 2869 个完整可行事件、2764 个逐单元去重解。独立验收逐解复算目标、
  成员账本、参与条件与 F 最小化选择，20/20 单元通过；嵌套候选不增加样本量。
- U 自然满足 θ=1 的比例仍为 0/20。20 个池的参与可行候选均为 0，故 F 全部按
  冻结定义回退到 I。公平成本 F 对 U 为 50c 2.325615%、100c 0.697952%、
  两算例行等权总体 1.511784%；F 对 U 的弱势成员收益改善总体均值 755.018 元。
- E6 结论为“不允许转移支付的参与保障下，本批候选空间内协同不可实施”，不是机制
  成功。正文须并列 E3 的约 1.48% 平均降本、少用车、里程与碳排上升，以及 50c
  广州约 875 元/12.6% 的收益损失。转移支付规则未实现，后续扩展须用户另批。
- 权威交付：
  `baselines/china_e3_e7/e6_fairness_v2_20260731/`；详细记忆：
  `docs/handoff/memory/e6_fairness_v2_20260731.md`；最终状态 `COMPLETE`。

## 2026-07-30 E6 v3 线程冻结后的独立终局

- v3 在
  `OMP_NUM_THREADS=OPENBLAS_NUM_THREADS=MKL_NUM_THREADS=VECLIB_MAXIMUM_THREADS=1`
  下从 0/20 独立执行，只重跑 JOINT 20 单元、ZONE 0 单元。20/20 终解哈希、
  完整目标位、实际评价数、trace 和终止原因与 E3 认证值逐项一致；搜索语义与六个
  保护文件未变。
- 2869 个完整可行事件、2764 个唯一候选经独立逐解复算通过；参与可行候选为 0，
  U 自然帕累托为 0/20，F 因候选池确实无可行参与解而在 20/20 单元回退 I。
  公平成本为 50c 2.325615%、100c 0.697952%、行等权总体 1.511784%；弱势成员
  收益改善总体均值 755.018 元。
- 管理解释继续保留 E3 的降本/少车和里程、碳排上升的双面权衡；不允许转移支付时，
  结论只限于这两个算例、共同 seeds、cap=400 和本批候选空间。任何 Shapley、
  MCRS 或其他转移支付规则仍须用户裁决。
- 权威目录：
  `baselines/china_e3_e7/e6_fairness_v3_20260731/`；详细记忆：
  `docs/handoff/memory/e6_fairness_v3_20260731.md`。独立验收、75 文件重封清单、
  AppleDouble 最终 0 和最后 `done.json` 全部闭合。

## 2026-07-31 E6 两中心 Shapley 事后结算

- 用户批准以 `e6_fairness_v3_20260731` 的 40 个已认证 I/U 状态行做事后
  收益分配，不重跑路线搜索。20 个配对单元的源哈希、成员账本和系统闭合复核通过，
  `search_reruns=0`。
- 无转移口径继续并列报告：自然帕累托 0/20、F 回退 I 20/20、两算例行等权
  公平代价 1.511784%。允许预算平衡内部转移后，19/20 个单元核非空；Shapley
  转移均值 817.374757 元，每名成员相对 I 的净改善均值 22.679075 元。
- 100c seed 4 的系统节省为 -1.135377 元，核为空。处理未使用外部补贴或跨单元
  补偿。权威目录：
  `baselines/china_e3_e7/e6_allocation_20260731/`；详细记忆：
  `docs/handoff/memory/e6_allocation_20260731.md`。

## 2026-07-30 E3 自然错配三臂重启的零搜索 HALT

- 用户批准恢复被误删的错配研究问题。81 实例全量输入审计确认 6 个非零错配实例、
  24/5805 个错配客户；六实例全部进入 IND/ZONE/JOINT、种子 1--10 的搜索前
  预注册，预算由 200c-01 JOINT seed1 的长探针确定。
- 正式搜索尚未启动时，预注册源合同
  `experiment_contract_v2_journal_aligned_20260730.md` 被外部写入，SHA-256
  从 `075f0093...f503b5e` 变为 `0e10e8ea...742c35`。监控按硬门暂停，
  probe/formal 状态行和预算锁均不存在。
- 终态为 `HALT_SOURCE_CONTRACT_HASH_DRIFT_BEFORE_SEARCH`。输入审计和预注册
  有效保留，三项正式效应为 null；后续须用户指定合同权威版本，并在新 sibling
  目录重启，禁止覆盖本轮 HALT 证据。
