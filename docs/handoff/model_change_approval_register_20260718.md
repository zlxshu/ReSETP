...
## 2026-07-28 E3/E6 执行绑定四项 `APPROVED_BY_USER_20260728`

> **批准记录**：2026-07-28 用户在对话中逐条批准 D2/D3/STAT，并在 Claude 解释"机制对照需等量、
> 算法比赛需收敛"的区别后批准 D4，附加条件原话：「要合适，不能把两边饿着了，饿着了是跑不出真成绩的」。
> 此前 Claude 曾越权自批并被用户纠正，该过程留痕于本节历史版本。


- `D2-A` **已批准**：实体车队按确定性输入驱动规则推导——逐场按 `(due_time, ready_time, customer_id)`
  固定排序做首次可行装箱得 `R_d`，`num_cv(d)=R_d`，`num_ev(d)=max(1, ceil((rho-1)R_d))`，
  主情景 `rho=1.25`，预注册敏感性 `rho ∈ {1.10, 1.25, 1.50}`。
  **附加条件（Claude 追加）**：`R_d` 必须在**新车型对**（EV 1700 kg / 77.28 kWh）下重算，
  旧 1000 kg 车型推出的车队权威一律作废，不得复用。
  充电设施主情景每场 `2×22 kW`，敏感性枪数 {1,2,3} × 功率 {11,22,44} kW，
  全部标注 `CONSTRUCTED_SCENARIO_NOT_OBSERVED_SITE_CONTRACT`，论文正文须显式写明为构造情景。
- `D3-A` **已批准**：对照臂采用多维容量硬锁车场归属（客户 delivery = 需求量 + 所属车场 one-hot），
  叠加完整候选守卫、路线池过滤与最终证书守卫四层。**执行前必须在当前源码上重放零搜索门**，
  2026-07-24 旧门因 adapter/epochal_hgs/route_pool_sp 哈希漂移已失效，不得以旧 PASS 替代。
- `D4-A` **已批准（附加防饥饿硬条件，见下）**：配对臂使用同一确定性完整候选评价预算（80 次），单线程，墙钟仅作熔断不作停止条件，
  进程 CPU 单独记录。正式跑前做结果盲 80/56/32 吞吐 pilot 定档，pilot 不得查看任何处理效应。
- `STAT-A` **已批准**：27 个地区--规模单元作**分层描述性**报告，不无条件当作 IID 样本；
  同地区不同规模共享客户 OSM 身份与区域基础设施的伪重复风险须在正文写明。
  目标刊机制章惯例不做统计检验（8 篇样本中仅 1 篇且仅用于算法比较），故本项以分层矩阵表为主，
  不以 p 值作为成败判据。

### D4 附加防饥饿硬条件（用户 2026-07-28 指示，具约束力）

等量预算只解决"归因"问题，不保证"两边都发挥出真实水平"。若 80 次对两臂都不够，
得到的是两个都没跑开的解，差值毫无意义。因此结果盲 pilot 的判据由"能不能跑完"改为
**"够不够"**，具体为：

1. 每臂每单元记录最后一次严格改进所在的评价序号 `L` 与总评价数 `S`，计算 `L/S`。
2. **非饥饿判据**：某一档预算下，任一臂中 `L/S > 0.5` 的单元占比超过 20%，即判该臂在该档
   **仍在改进中被截断**，该档不合格。
3. 选档规则改为：在 80/56/32 中选**满足非饥饿判据的最小档**；**若三档全部不合格，必须向上加档
   （160、240 …）直到满足，严禁向下取档或接受饥饿档**。
4. 两臂必须使用同一档；若两臂的最低非饥饿档不同，取二者中较大者。
5. pilot 全程结果盲：只看收敛形状与吞吐，不得查看任何臂间成本差异。

以上四项在 E2 洁净批完全释放算力之前不得启动正式搜索；D3 零搜索门重放与 D4 吞吐 pilot 均排在 E2 之后。

## 2026-07-28 E2 洁净批并发由 4 提升至 6（用户 2026-07-28 对话批准）

- **变更对象**：`baselines/e2_rerun_unified_01_20260727/run_unified_campaign.py` 第 67 行
  `WORKERS = 4` → `WORKERS = 6`；同目录 `monitor.json` 的 `required_command_substrings`
  由 `--workers 4` 同步改为 `--workers 6`。两文件均为受保护实验合约文件，改前已备份为
  `*.bak_workers4_20260728`。
- **批准依据**：用户 2026-07-28 明确批准（原话：批准），诉求为"必须跑完，不能偷工减料"下的提速。
- **技术判断**：本机 8 逻辑核。4 worker 时实测各约 92%--98% CPU、合计约 3.9 核，桌面进程约 1 核，
  空闲约 3 核。提升至 6 worker 后实测各约 84% CPU、合计约 5.0 核，仍为每 worker 接近独占一核，
  按墙钟计时的限时 MIP 单位墙钟可用算力基本不变。与 2026-07-27 事故（12 worker 抢 8 核、
  每 worker 仅约 30% CPU、限时 MIP 被系统性削弱）性质不同。
- **必须披露**：本批次为**中途变更并发**（前 2242 个尝试单元在 4 worker 下完成，其后在 6 worker 下完成）。
...
## 2026-08-01 E3--E7 文献重建正式分支待审批

状态：`AWAITING_USER_APPROVAL`。完整事实与短试边界见
`docs/handoff/memory/e3_e7_rebuild_foundation_20260801.md`。

- **E3/E6 车队**：候选为 Soriano 原文的每场 40 辆同质燃油车，或保留 China81 现有混合车队。
  临时 `40 CV + 10 EV/场` 没有外部数量依据，不得自动转正。
- **E4**：候选为文献常见的满电离场，或保留现有 0 电量日初预充；同时待批用“成本基线 vs
  运营成本+碳成本联合路径/车辆/站点/时刻优化”取代旧纯碳固定路线主实验。
- **E5**：正式算例保持 50c-01/100c-02；待批只采用 Montoya 的 `22 kW/16 kWh` 主参数，
  或再加入 Xiao 的 `16/20/24/32 kWh` 全套容量敏感性。
- **E6**：方法 A 事后结算与方法 B 搜索内利润下限已由用户批准同时比较；仍待批平衡/不平衡
  客户所有权的正式覆盖范围，以及逐客户抽象摩擦或车场对/实体运输成本两种非零费用语义。
- **E7**：触发规则与拒单收入损失已批准；仍待批按邱莹莹母体同比例生成新增、取消、需求减少三类
  事件，或只生成新增订单。

上述选择批准前不得启动正式搜索、替换论文主结果或写成已决定。旧 `0/10/25/50/95` 逐客户费用
没有直接现实/论文标定，不因历史记录自动恢复。

## 2026-08-01 E3--E7 重建实施裁决

状态：`APPROVED_FOR_IMPLEMENTATION_AND_SYMMETRIC_PILOTS`。本节取代上一节的
`AWAITING_USER_APPROVAL`，但不把任何试跑预写成正式正结果。

- **E3**：沿用 China81 算例已经给出的各车场、各车型数量上限；不再另定油电车辆比例，
  实际使用车型和数量由算法决定。采用散乱承包商客户归属作为合作前基准，合作后允许算法重分工。
- **E4**：保留旧零电量、固定路线纯碳择时结果；另行试验 Miyabe 式“带电出发、日末恢复”方案，
  并让路线、车辆、充电站和充电时刻共同优化。新旧方案全部保留；新方案是否升为正文主方案，
  由其合理性、故事完整性和实测效应共同判断。
- **E5**：正式算例仍为 50c-01 与 100c-02。采用 Montoya 的 `22 kW/16 kWh` 主比较，
  并补齐 Xiao 的 `16/20/24/28/32 kWh` 容量轴。车辆参数只需有文献范围支撑并在审稿视角下合理，
  不再要求逐项对应某一量产车型；允许使用明确标为实验设定的合理虚拟车辆，不再以“真实车型”作卖点。
- **E6**：方法 A（先做系统优化、事后结算）与方法 B（搜索时保障承包商不比单干差）同时保留；
  订单可直接转移的场景与需要车场间实体转运的场景做对称低成本试验，全部结果留痕。
  正式业务故事由用户查看试验结果后决定，不由执行代理代选。
- **E7**：按邱莹莹母体同比例生成新增、取消和需求减少事件；触发采用“累计到阈值则提前处理，
  否则最多等待 30 分钟”，同时保留逐单和固定 30 分钟比较。允许启用车型上限内未使用车辆；
  采用订单级拒绝和收入损失；以全知静态方案作参考，不再使用冻结旧方案硬扛。

实施纪律：先做完整留痕的对称小试，再决定正式算力投入；不得按结果删候选、种子或不利行。
机械修复和已批准方案实现由代理自主推进；任何会再次改变上述业务含义的选择再交用户拍板。

## 2026-08-01 E3--E7 对称小试完成及复核纠正

状态：`PILOTS_COMPLETE__E3_E4_E6_CANDIDATES__E5_FAILED__E7_METHOD_DECISION`。本节登记执行与复核结果，不替用户选择正式路线。
完整事实、文献判断和选项见
`docs/handoff/memory/e3_e7_rebuild_foundation_20260801.md`，当前未决项见
`docs/handoff/open_issues_20260731.md`。

- **E3**：原车型上限内由算法选车；150c-01 三种子“允许算法重分客户”相对“历史客户归属固定”平均降本 37.8234%。两臂同预算、均合法；旧技术标签 `HISTORICAL_STANDALONE` 不再用于论文。E6 四家 singleton 拼接有 2 处公共充电冲突，不能替代 E3 基准。
- **E4**：联合方案运营成本不变、系统排放下降 2.8187%；纯碳方案排放下降 14.3187%、
  运营成本上升 2.1990%。旧固定路线结果仍完整保留。临时 hook 泄漏已修复，隔离重跑科学字段一致；2.8187% 只作点结果，不称最大值。
- **E5**：FS/L1/L2/PL 共 120/120 合法，但 FS 的实际充电全部位于与 PL 完全相同的第一段；约 0.1% 差异来自启发式搜索波动，当前版本判失败。
- **E6**：15/15 联盟合法，大联盟降本 35.379%，Shapley 在核内；自然账本四家均优于单干，
  实体调货尚未形成闭合模型。
- **E7**：50 客户十种子 40/40 合法；混合触发相对固定 30 分钟平均低 1.73%。当前子集穷举
  不能扩到 100/150 客户。逐页核查未找到“批内永久拒单且不穷举”的现成文献算法；逐单到达顺序与批内顺序搜索两个实现方向须由用户选择，不能拼接后冒充照搬文献。

以上小试没有自动转为正式论文结果；E3--E7 的后续分支按未决清单 A-1 至 A-5 由用户拍板。

## 2026-08-01 E3--E7 正文主线裁决（用户明确批准）

- **E3**：`CLOSED_FORMAL_SUCCESS`。六算例×十种子正式批为唯一结果；不再新增研究分支。
- **E4**：`APPROVED_NEW_JOINT_MAINLINE`。路线—车型—充电联合优化升为正文主方案；旧固定路线结果只作补充。
- **E5**：`APPROVED_SENSITIVITY_RESEARCH_ONLY`。Montoya式小电池/时间受限问题只作敏感性；默认77.28 kWh、默认初始电量和人民币目标不替换。敏感性正式参数须在直接文献与历史排错完成后再交用户批准。
- **E6**：`APPROVED_COLLABORATIVE_SURPLUS_ALLOCATION_MAINLINE`。正式故事为合作创造总收益、独立基准、Shapley/核分配和参与检查；自然亏损、搜索内利润底线和实体调货不进入正文主线。
- **E7**：`APPROVED_QIU_PARENT_LOW_COST_TRIAL`。邱莹莹累计500 kg或最多30分钟规则是母体；允许补真实路线改动数并试验可扩展接单实现。外来方法不得自动取代母体。

本裁决不授权修改三份保护源码，不授权把E5敏感性改成全项目默认模型，也不授权把E7低成本试验直接写成正式优越性结论。

## 2026-08-01 E5--E7 基础设施纠正与执行登记

状态：`E5_OPTIONS_AWAITING_USER__E6_FORMAL_COMPLETE_ALLOCATION_CHOICE__E7_QIU_FORMAL_OPTIONS_READY`。

- **E5-B 仍待用户选择。** 用户已批准 E5 只作敏感性、China81 客户算例不变、默认 77.28 kWh 与默认人民币主模型不替换。两轮文献和代码审计新增确认：充电量未进入联合决策、路线生成看不见曲线、站点“用过即永久排除”比正式时段容量检查更严、满电离场漏记出发电量、独立弱搜索混入解质量差异。正式敏感性现分为 Montoya 时间/可行性、Wang 混合车队联合优化、修正后默认人民币三项，尚未由用户选择，因此未修改 E5 代码、未启动新实验。完整选项见 `docs/handoff/e5b_literature_and_infrastructure_diagnosis_20260801.md`。
- **E6-A 正式批已完成。** 正式范围固定为 150c/200c 各三个四车场算例、种子 1--10、每单元 15 个联盟，只计算合作节省、Shapley 与核；方法 B、自然亏损、服务费和实体调货均不进入。单元预算沿用已批准 pilot09 的 100 次/视角、archive 8、组合求解 5 秒。60 个单元和 900 条联盟记录全部合法且清单复算一致；合作节省均值 33.018%、范围 28.442%--37.145%，60/60 核非空，Shapley 在核内 52/60。Shapley 外核的 8 个场景均为三家公司联盟退出激励，差额 1.38--124.02 元。下一步只待用户在饶卫振 2022 的 Shapley 主分法与饶卫振 2019 的核仁主分法间选择；不得把任意核内可行点包装成正式制度。
- **E7 旧“订单级拒绝”授权被母体原文纠正。** 只读复核发现旧实现会主动拒绝整批本来可行的订单，偏离邱莹莹“所有新需求纳入重排”的母体。恢复整批接收属于修复对象错误，不是新增拒单制度。50c 十个固定事件流中，邱式 10/10 流、54/54 批合法且 100/100 新单全接；随后三触发同口径比较 30/30 合法，三种方式均 100% 接单。旧 1.73%/7.16% 混入经济筛单，停止引用。
- **E7 正式选项已经查清但尚未获用户选择。** 当前每阶段 8 次评价仅用于排障。邱的动态算例没有另给每次重排预算，且初始温度正文 200、参数表 500 冲突；姜的目标期刊论文给出三策略各 10 次，但其代数不能换成本项目评价次数。正式求解在“重建邱式 GSAA”与“保留本文主算法、先做本项目收敛曲线”之间选择；预知需求参照在“干净落实全天全部事件”与“按邱表格外观重建”之间选择。三触发才是正文现实策略，全信息方案只作参照，不得把启发式预知方案更差解释成动态信息优越。

## 2026-08-01 正式结果入稿与 E5/E7 参数授权边界更正

状态：`E3_E4_CLOSED__E5_B2_IMPLEMENTATION_PENDING__E6_FORMAL_ENDPOINT_PENDING__E7_SCALE_HORIZON_PENDING`。

- **DECISION/FACT（E3/E4/E6 入稿）**：用户要求 E5、E7 继续探索，其余已封存结论进入 TeX。`docs/paper_v2/paper_main.tex` 已写入 E3 六算例×十种子正式结果、E4 新联合优化主结果及旧固定路线补充结果、E6 四承包商 60 场景/900 联盟结果；旧 E5 零效应段和旧 E7 空结果表已撤下。没有改求解器、算例或实验参数，也没有启动新实验。
- **DECISION（E5）**：用户已在 E5-B 内选择 `E5-B2`，即保留 China81 客户、站点、原车型和默认主模型，另做 Wang 式路线—车型—充电联合优化敏感性。此前“E5-B1/B2/B3 仍待选”的登记由本条取代。仍未授权：同车同路重复到站采用访问编号还是站点副本、75 元/小时是否进入敏感性目标、正式求解预算。
- **FACT（E5 对象纠正）**：当前 E5 不存在跨路线永久禁站；120 个已保存方案中已有 24 个方案由不同车辆错峰复用同一物理站。真实缺口是同一条路线重复到达同一物理站时缺少无歧义表示。不能通过直接删除判断修复，否则保存、复算和证书无法区分两次到站。
- **DECISION/FACT（E7）**：用户批准的是邱莹莹“累计量达到阈值即提前处理，否则最多等待 30 分钟”的触发形式和整批接单；用户随后明确要求 500 kg 只模仿取值思路，不能原样照搬到不同车辆、规模和 7×24 小时业务。因此此前“500 kg 固定”的登记被本条部分取代，具体公斤数、收尾时刻和事件接收窗均未批准。仓库冻结的 China81 默认服务时段实际为 06:00--22:00；东莞 7×24 来源只证明海关通关，不证明普通配送全天运营。E7 若采用连续 24 小时或 168 小时周，必须另建并标明 E7 专属情景，不能静默替换项目默认时域。
- **FACT（E7 新证据）**：邱第 47 页五张新增订单货量为 223/258/243/246/251 kg，按其 08:03/08:17/08:59/09:14/09:43 到达与 30 分钟时钟分批后，500 kg 从未真正触发。当前代码把五个时刻在 50/100/150 客户算例中原秒复制 2/4/6 次，导致规模越大越容易因同秒堆单越过阈值。旧 50c 三触发结果只保留为程序诊断；正式 E7 还需用户选择连续运营时域、事件生成、累计阈值、算法/预算和预知需求参照。
- **VERIFICATION**：XeLaTeX 生成 24 页 `paper_main.pdf`；数学审计 `36 PASS / 1 WARN / 0 FAIL`，唯一警告是若干公式标签尚未在正文引用；新增排版错误 0。TeX/PDF SHA-256 为 `d662959a0b8086cdc4b31c211899267128c429c322f5cfb2a4b55086b18dceb6` / `8dd6a6ec49028aa35ee604955e8fca24dd5c9587c5ccc38deab284c82bd4029f`。

## 2026-08-01 E5-P1/R2、E6-S2、E7-H0/G2 执行及 E7 触发对象待批

状态：`E3_E4_E6_CLOSED__E5_B2_FOUNDATION_READY__E7_TRIGGER_OBJECT_PENDING`。

- **DECISION/FACT（E5）**：用户选择 `E5-P1` 与 `E5-R2`。原车型、客户、站点、默认初始电量和项目默认人民币模型保持不变；75 元/小时只进入 E5-B2 敏感性。访问副本映射回同一物理充电站，容量检查仍按物理站合并，没有增加充电枪。50c-01 零搜索真实构造得到 9 条路线、3 次充电、0 项违反。该结果只证明新表示可运行，尚未证明非线性充电产生效应。
- **DECISION/FACT（E6）**：用户选择 `E6-S2`。核仁直接由原 60 个完整联盟账本后处理得到，没有重跑车辆路径。60/60 场景的核仁均在核内，240/240 公司--场景均高于单干利润，最小增益 265.998 元；Shapley 保留为贡献分配参照，在核内 52/60。E6 关闭。
- **DECISION/FACT（E7 基础）**：用户选择 `E7-H0/G2`。活动场景保留 China81 的 06:00--22:00 服务日；50/100/150 客户分别生成 10/20/30 个独立出现订单，并保证每单出现早于自身最早服务时刻。旧 24 小时活动分支和旧同秒复制生成器不再作为活动入口。
- **FACT（E7 对象纠正）**：邱莹莹第 23--24 页的公式和当前程序触发的是“处理新信息并重新计算尚未执行的路线”，不是“车辆在触发时立刻发车”。用户提出的“零星货发车会亏、久等损害服务”的经营逻辑有经济发车文献支持，但对应的是另一个动作。邱第 47 页披露 500 kg 与 30 分钟，没有披露 500 kg 的成本推导、调查或敏感性，展示算例中又始终由 30 分钟先触发。因此不得把 170 元车辆固定费用作电脑重算门槛，也不得替作者断言 500 kg 来自经验。
- **AWAITING_USER_APPROVAL（E7 触发对象）**：`E7-O1` 保持“何时重算剩余路线”；`E7-O2` 改为“何时释放订单并让车辆真正发车”；`E7-O3` 分成重算和发车两层。若选 O2，再在目标期刊的简洁经济规则 `E7-D1` 与含客户位置和预计路线成本的 `E7-D2` 间选择；两者都需要有出处的等待损失参数。批准前不写入公斤门槛、不启动正式 E7 实验。证据与选项见 `docs/handoff/e7_economic_dispatch_threshold_options_20260801.md`。
- **VERIFICATION**：E5-P1/R2 与 E7-H0/G2 聚焦测试 `9 passed in 2.94s`；E6-S2 测试 `2 passed in 1.09s`，五件套四个登记产物哈希均匹配。XeLaTeX 成功生成 24 页 PDF，未定义引用 0；`git diff --check` 通过。

## 2026-08-01 E7 O1→O2→O3 顺序试验授权

状态：`E7_O1_LOW_COST_TRIAL_AUTHORIZED`。

- **DECISION**：用户明确要求按顺序逐条尝试：先做 `O1` 重算路线；O1 不形成效应再做 `O2` 真实发车；O2 仍不形成效应再做 `O3` 双层。研究目标是形成可发表且能解释的实验效应。
- **边界**：三条路线顺序执行，不并行跑完后挑最有利者；所有已运行算例、种子和不利结果保留。不得自行增加百分比合格线、显著性门槛或结果过滤规则。“不行”按完整结果是否稳定运行、各方式是否实际产生不同决策、差异是否能用目标期刊的成本/里程/车辆/响应/路线调整叙事解释综合判断，不用代理自建单一数值门槛。
- **O1 当前依据**：邱的 500 kg 约等于其 5 张新增订单平均货量的 2.05 倍，也等于其 1000 kg 车型载重的一半；原文未说明采用哪条理由。因为 O1 触发的是信息重算而不是发车，本轮先按“约两张典型订单的累计需求”迁移，即每个算例用其完整客户需求均值的两倍，并保留 30 分钟上限。该迁移是透明的项目推断，先做对称低成本机制试验，不直接转为正式参数。
- **已完成零算力检查**：两倍平均订单需求在 50/100/150 客户算例分别为 488.92/559.82/536.15 kg。10 个固定流中，数量分支分别在 4/10、10/10、10/10 个流实际触发，共触发 5/33/74 次；因此三种规模均可检查数量机制，且不再原秒复制邱的五个时刻。850 kg 的半车容量迁移在 50 客户 10/10 流均从未触发，不能用来检验 O1 的数量分支。

## 2026-08-01 E2--E7 源码复核后的新增待批事项

状态：`AUDIT_FACTS_RECORDED__NO_NEW_RESEARCH_DECISION_ASSUMED`。

- **FACT（全局多趟冲突）**：E3、E4、E5、E6正式求解入口均没有调用仓库已有的多趟装车程序，正式保存解实际为一路一车；论文却把同车多趟写成已实现。现有数字仍是同一单趟口径下的真实配对结果，但不能同时充当完整多趟模型的证据。
- **FACT（不能直接补开关）**：90份E4保存解虽都能生成多趟排班证书，但通用接口漏掉270笔返场补电；900份E6联盟解中897份通过完整检查、3份发生第二趟电动车补电冲突。多趟若保留，需专门修复并重跑E3/E4/E6，E6还须重算全部联盟价值和核仁。
- **AWAITING_USER_DECISION（M1/M2）**：`M1`保留现有正式数字，把论文和模型明确收窄为单车单趟；`M2`保留多趟为核心能力，先补接口与记账，再重跑受影响实验。任何代理不得提前修改论文多趟主张或把旧数字改标为多趟结果。
- **AWAITING_USER_DECISION（E5）**：获批B2/P1/R2均已实现并运行，但原车型下两曲线差异仅约`-0.000003%`且途中充电为0。停止E5、增加有直接文献依据的95 km敏感性、或随M2先补真实多趟，三者改变正式比较面，须用户选择。
...
- **FACT（E2旧车型）**：E2的O/F/E/M/MV均锁同一`china81.py` SHA-256 `df082b6a...fcedd`，参数为140.41 kWh、载重1000 kg、整备3300 kg、迎风面积6.0775 m²。五种方法内部比较公平，354胜/46平/5负仍有效。
- **FACT（正文错配）**：正文把E2与后续机制实验一并写成现行77.28 kWh、载重1700 kg、整备2600 kg车型，车型表EV迎风面积却仍是旧6.08 m²。不能只改一处数字、删掉两对重放失败或重算六条充电动作来冒充现行车型实验。
- **AWAITING_USER_DECISION（E2-V1/V2/V3）**：V1保留旧车型结果并明确两套实验参数；V2按现行车型完整重跑五种算法；V3撤下China81算法主结果。代理不代选。
...
- **车场充电并发变更前位置与合同**：`solver/src/setp_solver/china81.py:573-579`，其中原 `:578` 把车场输入 `configured_depot_gun_count_if_finite=2` 绑定到 `Node.station_chargers`；受保护的 `solver/src/setp_solver/check.py:504-550`（容量读取原 `:538`，静态容量函数 `:1120-1128`）以及 `solver/src/setp_solver/search/dynamic_multitrip_schedule.py:1352-1408`（静态容量读取原 `:1397-1403`）据此执行有限并发检查。
- **车场充电并发变更后位置与合同**：在 `solver/src/setp_solver/model_config.py` 增加显式配置，默认模式为车场并发不设上限；`solver/src/setp_solver/china81.py` 在默认模式下把车场 `Node.station_chargers` 置为 `None`，从而复用现有检查器的非约束性静态容量回退。旧的实例有限容量模式保留为可选配置并继续读取 `configured_depot_gun_count_if_finite=2`，供旧包复现与 Froger 式敏感性使用。受保护的 `check.py`、`search/evaluation.py` 均不改；公共站构造 `china81.py:581-594` 及其 `station_gun_count` 语义不改。
