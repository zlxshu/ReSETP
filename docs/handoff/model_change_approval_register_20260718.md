...
状态：`PARTIALLY_APPROVED_PRINCIPLES_ONLY`（2026-07-18）。
...
状态：`DEFERRED_NOT_APPROVED_PENDING_BLOCKERS`。
...
状态：`CONDITIONALLY_APPROVED_CANDIDATE_A`（2026-07-18；重庆身份池与27格充足性门未过前不得正式应用）。
...
- **MC-004 = 暂不批准**（维持登记建议）。九城货车入口、本地冻结 OSM、路由器版本、货运 profile 闭合后按本地路由器重提。
