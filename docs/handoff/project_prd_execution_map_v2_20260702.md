...
> 2026-07-11 执行入口更新：本PRD保留完整技术背景和历史gate；面向7.31投稿的白话总实验方案、E0--E7详细任务、阶段门和日程，以 `docs/handoff/project_experiment_master_plan_20260711.md` 为当前施工入口，实时状态看 `docs/handoff/project_board_20260711.md`。同日用户后续裁决已将280 kWh现代配送场景设为投稿正面主场景，覆盖本文较早的“Goeke80唯一正式主场景”历史口径；80 kWh保留为独立稳健性基准。E2全量最新判决见 `docs/handoff/e2_full_closeout_20260711.md`。

> **2026-07-11最终覆盖：** 当前先完成E2的九算例×九算法×十次×4000评价统一比赛，再立即准备E3。旧270行不是终点，只复用其中225行合格主表数据；补齐后共810行。E2不另设完整模型复核层，公平关闭、跨场费0、280 kWh主场景。BKS/AVG/Gap%与未经改动Goeke原始算法留作E2后的公开标准算例补实验。客户归属、非零跨场费和线性碳配额继续研究挂账。完整拍板以`docs/handoff/e1_e7_submission_contract_decision_20260711.md`为准；本文件后续旧路线只作历史。
...
旧E2的270/270健康矩阵是可复用前置证据，不再视为最终闭合。最终E2要扩为九个健康算法、每组十次的810行统一比较；完成后关键路径立即转E3。本文后续仍保留的E2旧门段落均为历史决策链，不得覆盖最新拍板。
...
**最新覆盖：** 下述“E2算法性能闭合优先”是2026-07-11早期挂账时的历史状态。E2和15负有界恢复现已收口，80 kWh最小镜像门已解锁。遗留编号1--6、9的当前权威顺序只见`docs/handoff/e2_legacy_items_1_6_9_execution_plan_20260711.md`；本节保留为决策历史，不再提供实时调度。
