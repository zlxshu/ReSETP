...
def _trip_rows() -> list[dict[str, object]]:
    ...
    for route in evaluation.prepared_solution.routes:
        ...
        rows.append({
            ...
            "vehicle_type": route.vehicle_type.upper(),
            "departure_return": f"{_clock(schedule.departure_second)}--{_clock(schedule.return_second)}",
            "distance_km": energy.distance_m / 1000.0,
            "cost_cny": fixed + distance_cost + fuel_cost + electricity_cost + carbon_cost,
            "energy": f"{energy.fuel_liters:.2f} L" if route.vehicle_type.lower() == "cv" else f"{energy.ev_drive_kwh:.2f} kWh",
...
def _write_trip_rows(rows: list[dict[str, object]]) -> None:
    ...
    latex = []
    for row in rows:
        route_id = row["route_id"].replace("_", r"\_").replace("#", r"\#")
        latex.append(
            f"{route_id} & {row['customers']} & {row['vehicle_type']} & "
            f"{row['departure_return']} & {row['distance_km']:.2f} & {row['cost_cny']:.2f} & "
            f"{row['energy']} & {row['emissions_kg']:.2f} \\\\"
        )
...
def _build_carbon_charging_figure() -> None:
    ...
    fig, axes = module.plt.subplots(2, 1, figsize=(4.30, 5.20), sharex=True)
    fig.subplots_adjust(left=0.15, right=0.84, top=0.96, bottom=0.10, hspace=0.28)
    signal_specs = (
    ...
    for ax, signal, ylabel, panel in signal_specs:
        load_ax = ax.twinx()
        width = 0.36
        load_ax.bar(hours - width / 2, asap, width=width, color=module.PALETTE["purple"], alpha=0.72)
        load_ax.bar(hours + width / 2, aware, width=width, color=module.PALETTE["amber"], alpha=0.82)
        ax.step(
            module.np.arange(25),
            module.np.r_[signal, signal[-1]],
            where="post",
            color=module.PALETTE["blue"],
        ...
        handles = [
            module.Patch(facecolor=module.PALETTE["purple"], alpha=0.72),
            module.Patch(facecolor=module.PALETTE["amber"], alpha=0.82),
        ]
        labels = ["有空即充", "碳感知充电"]
    ...
    axes[1].set_xlabel("时刻", fontproperties=module.CN)
...
def _build_fleet_carbon_figure() -> None:
    ...
    module = _approved_figure_module()
    fig, ax = module.plt.subplots(figsize=(4.30, 2.80))
    ...
    ax.plot(prices, cv_counts, color=module.PALETTE["blue"], marker="o", markersize=3.0, markerfacecolor="white", linewidth=module.CURVE_WIDTH_PT)
    ax.plot(prices, ev_counts, color=module.PALETTE["red"], linestyle="--", marker="^", markersize=3.2, markerfacecolor="white", linewidth=module.CURVE_WIDTH_PT)
    ax.set_xlim(0, 2.2)
...
def _build_collaboration_figure() -> None:
    root = REPO / "solver/reports/paper_submission_preview_20260828/missing_static/collaboration"
    left = root / "independent/seed_11/best_solution.json"
    right = root / "joint/seed_11/best_solution.json"
    if not left.is_file() or not right.is_file():
        return
    module = _approved_figure_module()
    module.build_route_figure(
        "figure_5_standalone_joint_routes_preview",
        "图5  不同配送模式的最终路径",
        "独立配送",
        "联合配送",
        module.load_route_sequences(left),
        module.load_route_sequences(right),
    )
