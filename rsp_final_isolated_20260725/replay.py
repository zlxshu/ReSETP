"""Independent replay and frozen G0 decision for the isolated package."""
...
def replay(payload: dict, bundle: object, expected: float, label: str) -> float:
    solution = runtime.load_solution(payload)
    objective, _, exact_violations = exact_china81_score(solution, bundle)
    direct_violations = check_solution(solution, bundle.instance, bundle.prices)
    ...
    if not math.isclose(
        float(objective), float(expected), rel_tol=0.0, abs_tol=1.0e-9
    ):
        raise RuntimeError(f"{label} replay objective drift: {objective} != {expected}")
    return float(objective)


def main() -> int:
    registration = runtime.verify_registration()
    with (runtime.OUTPUT / "raw_runs.csv").open(
    ...
    replay_rows = []
    for row in rows:
        witness = runtime.read_json(runtime.REPO / row["witness_path"])
        bundle = runtime.load_bundle(row["instance_id"])
        start = replay(
            witness["start"],
            bundle,
    ...
    runtime.write_csv(runtime.OUTPUT / "independent_replay.csv", replay_rows)
    by_instance = {}
    for row in rows:
    ...
    improvement_005 = sum(
        float(row["improvement_pct"]) >= 0.05 for row in resource
    )
    rsp_beats_local = sum(
    ...
    decision = {
        "schema": "resetp.resource-slot-pricing-decision.v4",
        "verdict": verdict,
        "pass": passed,
    ...
    runtime.write_json(runtime.OUTPUT / "decision.json", decision)
    (runtime.OUTPUT / "report.md").write_text(
        "# Resource-slot pricing isolated G0\n\n"
        f"Verdict: `{verdict}`.\n\n"
    ...
    runtime.write_json(
        runtime.OUTPUT / "artifact_hashes.json",
        runtime.artifact_hashes(runtime.OUTPUT),
    )
    runtime.write_json(
        runtime.OUTPUT / "done.json",
        {
            "verdict": verdict,
