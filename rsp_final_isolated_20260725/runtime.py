"""Paths, frozen-input verification, and artifact helpers without path pollution."""
...
REGISTRATION = PACKAGE / "g0_registration_v4.json"
ENGINEERING = PACKAGE / "engineering_gate_v4"
OUTPUT = PACKAGE / "g0_gate_v4"
AUTHORITY = (
    REPO
...
solver_source = str(REPO / "solver/src")
if solver_source not in sys.path:
    sys.path.append(solver_source)
...
def verify_registration() -> dict[str, Any]:
    ...
    for item in registration["protected_hashes"]:
        path = REPO / item["path"]
        if not path.is_file() or sha256(path) != item["sha256"]:
            raise RuntimeError(f"protected input drift: {item['path']}")
