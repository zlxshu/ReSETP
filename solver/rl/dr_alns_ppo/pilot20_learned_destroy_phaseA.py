...
def run_phase_a(args: argparse.Namespace) -> int:
    ...
    return 0 if verdict["status"] in {"PASS", "WEAK"} else 2
...
def _verdict(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if zero_violations and avg >= 2.0 and min_scale >= 0.0 and (not random_improvements or avg_vs_random > 0.0):
        status = "PASS"
    ...
    return {
        ...
        "rule": "PASS if held-out avg >= 2%, every scale >= 0%, learned beats random on average, and learned has zero violations; WEAK if 0-2%; otherwise HALT.",
    }
