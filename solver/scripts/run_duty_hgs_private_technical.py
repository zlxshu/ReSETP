#!/usr/bin/env python3
"""Run one bounded real-input trial of the formal self-developed Duty-HGS.
...
from setp_solver.algorithms.duty_hgs.evaluation import (
    DutyEvaluationContext,
    DutyFullEvaluator,
    FrozenMappingIdentity,
    mapping_sha256,
)
from setp_solver.algorithms.duty_hgs.model import DutyIndividual, PhysicalVehicleDuty
from setp_solver.algorithms.duty_hgs.operators import (
    ReverseSegmentMove,
    generate_problem_moves,
)
from setp_solver.algorithms.duty_hgs.population import (
...
def main() -> int:
    ...
    if not crossover_changed:
        failure_reasons.append("DCREX did not change its selected main parent")
    if (
    ...
    metadata = {
        ...
        "failure_conditions": [
            ...
            "DCREX does not structurally change its selected main parent",
