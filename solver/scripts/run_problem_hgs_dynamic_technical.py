#!/usr/bin/env python3
"""One future-only wiring trial of the formal self-developed Problem-HGS.

This runner checks that the same route core and problem-mechanism
...
def _dynamic_source_provenance(repo: Path, output: Path) -> dict:
    """Extend the shared formal-package provenance with this runner."""
    ...
    manifest[str(runner.relative_to(repo))] = _sha256(runner)
    payload = json.dumps(
        manifest,
        ensure_ascii=False,
        sort_keys=True,
    ...
    provenance["python_source_sha256"] = hashlib.sha256(payload).hexdigest()
    return provenance
...
def _dynamic_state(initial, static_evaluation, bundle, trigger_second: float):
    ...
    return (
        DutyDynamicState(
            ...
            future_customer_ids=frozenset(customers.difference(committed_customers)),
...
def main() -> int:
    ...
    if not 0.0 < args.max_runtime_seconds <= 1200.0:
        raise ValueError("maximum runtime must be in (0, 1200] seconds")
    if args.stagnation_patience < 1:
        raise ValueError("stagnation patience must be positive")

    repo = Path(__file__).resolve().parents[2]
    output = args.output_dir.resolve()
    provenance = _dynamic_source_provenance(repo, output)
    if output.exists():
    ...
    source_solution = complete_china81_route_skeleton(
        source_solution,
        bundle,
    ).solution
    customer_ids = {
    ...
    result = run_integrated_problem_hgs(
        ...
        stop=lambda search_state: (
            search_state.iterations >= args.iterations
            or search_state.elapsed_seconds >= args.max_runtime_seconds
        ),
    ...
    if result.termination_status not in {
        "STOPPED_BY_CALLER",
    ...
    metadata = {
        ...
        "stop_semantics": (
            "technical iteration/convergence stop with a hard runtime ceiling"
        ),
        ...
        "synthesized_idle_asset_rule": {
            ...
            "ev_remaining_battery_kwh": float(bundle.prices.initial_ev_battery_kwh),
            "next_trip_index": 1,
        },
    ...
    row = {
        ...
        "committed_history_preserved": (history_before_sha256 == history_after_sha256),
