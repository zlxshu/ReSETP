...
def trip_assignment_exchange(
    ...
    The receiving physical vehicle keeps its identity, type, home depot, and
    dynamic history; the affected unlocked charging decisions are rebuilt by
    the caller under the complete model.
    """
    ...
    for donor_duty in sorted(
        ...
        for donor_trip in donor_duty.trips:
            ...
            if (
                ...
                continue
            origins = {origin_duties[item] for item in donor_customers}
            receivers = tuple(
                duty
                for duty in first.duties
                if (
                    duty.vehicle_type == donor_duty.vehicle_type
                    and duty.home_depot_id == donor_duty.home_depot_id
                    and duty.physical_vehicle_id not in origins
                )
            )
            if receivers:
    ...
    donor_duty, donor_trip, receivers = candidates[rng.randrange(len(candidates))]
    ...
    receiver = min(
        ...
        key=lambda duty: (
            _squared_distance(
                _duty_centroid(duty, customer_coordinates, donor_centroid),
                donor_centroid,
            ),
            duty.physical_vehicle_id,
        ),
