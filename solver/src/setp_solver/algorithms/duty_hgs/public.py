"""Public MDVRPTW HGS using the final SREX/DCREX crossover portfolio.

Vanilla PyVRP 0.12.2 remains an untouched baseline.  This module is the
project algorithm: it reuses PyVRP's population, penalties, and compiled local
search, but owns the crossover portfolio, multi-parent DCREX, five insertion
actions, control loop, trajectory, and source-faithful convergence stopping.
"""
...
from pyvrp.search._search import Node as SearchNode
from pyvrp.search._search import Route as SearchRoute
from pyvrp.search._search import insert_cost
...
class _PublicInsertionWorkspace:
    """Reuse compiled PyVRP route objects throughout one DCREX repair."""
...
class PublicDCREXHGS:
    ...
    def __init__(
        ...
        *,
        ...
        pyvrp_rng: RandomNumberGenerator,
        random_seed: int,
        ...
        if version != "0.12.2":
            raise RuntimeError(
                f"formal public DCREX requires PyVRP 0.12.2 HGS, got {version}"
            )
    ...
    def _run(self) -> PublicDCREXResult:
        ...
        for iteration in range(1, self.parameters.max_iterations + 1):
            ...
            if crossover_action == CrossoverAction.FAST:
                ...
                parent_genes = tuple(
                    _solution_genes(solution) for solution in parent_solutions
                )
                crossed = dcrex_exchange(
                    parent_genes,
                    customer_universe=_customer_universe(
                        self.data,
                        parent_solutions,
                    ),
                    rng=self.rng,
                ...
                routes, missing = _remove_public_duplicates(
                    crossed.routes,
                    _customer_universe(self.data, parent_solutions),
                )
            ...
            trajectory.append(
                PublicDCREXTrajectoryRow(
                    ...
                    reward_per_work_unit=(
                        crossover_reward.reward_per_work_unit
                    ),
                )
            )
...
def build_public_dcrex_hgs(
    ...
    *,
    ...
    pyvrp_rng = RandomNumberGenerator(seed=int(seed))
    neighbours = compute_neighbours(data, solve_parameters.neighbourhood)
    local_search = LocalSearch(data, pyvrp_rng, neighbours)
    ...
    initial_solutions = tuple(
        Solution.make_random(data, pyvrp_rng)
...
def _public_search_route(
    ...
    """Build one PyVRP search route for exact compiled insertion deltas."""
