...
def run_duty_hgs(
    ...
    *,
    ...
    """Run Duty-HGS until DCREX converges or the external stop fires."""
    ...
    crossover_controller = CostAwareCrossoverController(
        gamma=float(parameters.dcrex_discount_factor)
    )
    ...
    while True:
        ...
        crossover_action = crossover_controller.select(rng)
        try:
            ...
            crossover_controller.update(crossover_action, 0.0, 1)
        ...
        raw_reward = percentage_reward(
            penalty_manager.cost(main_record.evaluation),
            penalty_manager.cost(educated_evaluation),
        )
        if diversity_level is not None:
            dcrex_controller.update_diversity(diversity_level, raw_reward)
        if insertion_operator is not None:
            dcrex_controller.update_insertion(insertion_operator, raw_reward)
        crossover_controller.update(
            crossover_action,
            raw_reward,
            deterministic_work_units,
