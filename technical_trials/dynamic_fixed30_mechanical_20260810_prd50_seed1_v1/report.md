...
本轮使用 `fixed_30_minutes` 只为检查连续接线，不代表已经选择正式触发政策。空闲电车待命充电口径为 `none`，只使用当时已经出现的客户，不读取后续隐藏订单；它仍是技术候选，不代表正式政策已经选定。待命充电累计 0.000000 kWh，计入后总成本为 2314.500897、总排放为 177.530692 kg。
