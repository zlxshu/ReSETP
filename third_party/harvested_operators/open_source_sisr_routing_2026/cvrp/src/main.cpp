...
int main(int argc, char *argv[])
{
    ...
    cxxopts::Options options("solver", "Open-source C++ implementation of Slack Induction by String Removals for capacitated vehicle routing problem");
    options.add_options()
    ...
    {
        if (!readProblemFileVRP(instance_path, problem))
        {
    ...
    if (cli_args.count("output-agg") != 0)
    {
        ...
        if (outFile.is_open())
        {
            outFile << filename_without_extension << " " << solver.sol_best->total_cost << " " << duration << std::endl;
            outFile.close();
    ...
    std::cout << "Best solution value: " << solver.sol_best->total_cost << " " << " Computation time (s): " << duration << std::endl;
