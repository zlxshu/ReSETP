...
std::ostream &operator<<(std::ostream &out, setp_hgs_kernel::Solution const &sol);

template <> struct std::hash<setp_hgs_kernel::Solution>
{
    size_t operator()(setp_hgs_kernel::Solution const &sol) const
    {
        size_t res = 17;
        res = res * 31 + std::hash<size_t>()(sol.numRoutes());
        res = res * 31 + std::hash<setp_hgs_kernel::Distance>()(sol.distance());
        res = res * 31 + std::hash<setp_hgs_kernel::Duration>()(sol.duration());
        res = res * 31 + std::hash<setp_hgs_kernel::Duration>()(sol.timeWarp());
