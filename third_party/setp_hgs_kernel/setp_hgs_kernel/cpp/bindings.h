...
template <setp_hgs_kernel::MeasureType T, std::integral V>
struct type_caster<setp_hgs_kernel::Matrix<setp_hgs_kernel::Measure<T, V>>>
{
    PYBIND11_TYPE_CASTER(setp_hgs_kernel::Matrix<setp_hgs_kernel::Measure<T PYVRP_PARAM_SEP V>>,
                         _("numpy.ndarray[int]"));
    ...
    bool load(pybind11::handle src, bool convert)  // Python -> C++
    {
        ...
        std::vector<setp_hgs_kernel::Measure<T, V>> data
            = {buf.data(), buf.data() + buf.size()};

        value = setp_hgs_kernel::Matrix<setp_hgs_kernel::Measure<T, V>>(
    ...
    cast(setp_hgs_kernel::Matrix<setp_hgs_kernel::Measure<T, V>> const &src,  // C++ -> Python
...
template <setp_hgs_kernel::MeasureType T, std::integral V>
struct type_caster<setp_hgs_kernel::Measure<T, V>>
{
    PYBIND11_TYPE_CASTER(setp_hgs_kernel::Measure<T PYVRP_PARAM_SEP V>, _("int"));
    ...
    bool load(pybind11::handle src, bool convert)  // Python -> C++
    {
        ...
        value = setp_hgs_kernel::Measure<T, V>(raw);
        return !PyErr_Occurred();
    }

    static pybind11::handle
    cast(setp_hgs_kernel::Measure<T, V> const &src,  // C++ -> Python
         [[maybe_unused]] pybind11::return_value_policy policy,
...
template <setp_hgs_kernel::MeasureType T, std::floating_point V>
struct type_caster<setp_hgs_kernel::Measure<T, V>>
{
    PYBIND11_TYPE_CASTER(setp_hgs_kernel::Measure<T PYVRP_PARAM_SEP V>, _("float"));
