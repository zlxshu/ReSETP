...
{
    setp_hgs_kernel::ProblemData const &data_;
    size_t depot_;
...
public:
    ...
    {
        setp_hgs_kernel::ProblemData::Depot const &depot = data_.location(depot_);
        return {depot};
    }
...
setp_hgs_kernel::Cost RelocateWithDepot::evaluate(Route::Node *U,
                                        Route::Node *V,
                                        CostEvaluator const &costEvaluator)
{
...
template <>
bool setp_hgs_kernel::search::supports<RelocateWithDepot>(ProblemData const &data)
{
