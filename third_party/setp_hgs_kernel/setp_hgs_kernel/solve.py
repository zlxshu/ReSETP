...
import setp_hgs_kernel.search
from setp_hgs_kernel.GeneticAlgorithm import GeneticAlgorithm, GeneticAlgorithmParams
from setp_hgs_kernel.PenaltyManager import PenaltyManager, PenaltyParams
from setp_hgs_kernel.Population import Population, PopulationParams
from setp_hgs_kernel._setp_hgs_kernel import ProblemData, RandomNumberGenerator, Solution
...
from setp_hgs_kernel.diversity import broken_pairs_distance as bpd
...
class SolveParams:
    ...
    def from_file(cls, loc: str | pathlib.Path):
        ...
        if "node_ops" in data:
            node_ops = [getattr(setp_hgs_kernel.search, op) for op in data["node_ops"]]

        route_ops = ROUTE_OPERATORS
        if "route_ops" in data:
            route_ops = [getattr(setp_hgs_kernel.search, op) for op in data["route_ops"]]
