...
def run_hgs_route_pool_recombination(
    ...
    *,
    ...
    max_no_improvement_iterations_per_view: (
    ...
    if isinstance(max_no_improvement_iterations_per_view, Mapping):
        if set(max_no_improvement_iterations_per_view) != set(modes):
            raise ValueError(
                "no-improvement mapping must cover exactly the three "
                "registered views"
            )
        no_improvement_limits = {
            mode: int(max_no_improvement_iterations_per_view[mode])
            for mode in modes
        }
    else:
        no_improvement_limits = {
            mode: (
                None
                if max_no_improvement_iterations_per_view is None
                else int(max_no_improvement_iterations_per_view)
            )
            for mode in modes
        }
    ...
    if all(limit is None for limit in iteration_limits.values()):
    ...
    if any(
        ...
        for limit in no_improvement_limits.values()
    ):
        raise ValueError("no-improvement limits must be positive")
